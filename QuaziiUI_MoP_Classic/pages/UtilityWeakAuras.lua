...
-- Modified parseWAData to handle index 2 (Utility WAs)
---@return table
local function parseUtilityWAData()
    local data = {}
    -- Check if utility WAs exist (index 2)
    if QuaziiUI.imports.WAStrings[2] and QuaziiUI.imports.WAStrings[2].WAs then
        -- Access category 2 WAs (utility)
        ...
        if info then
            ...
            if not WeakAuras then
                ...
                line.importButton:SetClickFunction( function()
                    -- Use category 2 string for import (utility WAs)
                    if QuaziiUI.imports.WAStrings[2] and QuaziiUI.imports.WAStrings[2].WAs then
...
function page:CreateDescription(frame)
    local text = QuaziiUI.DF:CreateLabel(frame, L["UtilityWAText"], QuaziiUI.PageTextSize) 
...
function page:CreateWAList(frame)
    ...
    frame.addonHeader = QuaziiUI.DF:CreateHeader(frame, headerTable, options, "QuaziiUIInstallUtilityWAHeader")
