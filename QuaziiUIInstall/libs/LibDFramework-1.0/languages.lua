...
local addonNamespaceOptions = {ChangeOnlyRegisteredFont = false}
local optionsHelp =
	{
...
local getByteCodeForCharacter = function(character)
	...
	for symbolPiece in
		character:gmatch(".") --separate each byte, can one, two or three bytes
	do
...
local triggerRegisteredCallbacksForAddonId = function(addonId)
	...
	if not addonNamespaceTable then return end
...
local registerTableKey = function(
addonNamespaceTable,
	table,
	key,
	phraseId,
...
function DF.Language.RegisterCallback(addonId, callback, ...)
	...
	addonNamespaceTable.callbacks[#addonNamespaceTable.callbacks + 1] = {
		callback = callback,
		payload = {...},
	}
	return true
...
---@param key string
---@return boolean
function DF.Language.DoesPhraseIDExistsInDefaultLanguage(
addonId,
	key --~DoesPhraseIDExistsInDefaultLanguage
)
	if not isValid_AddonID(addonId) then
		error(
			"DoesPhraseIDExistsInDefaultLanguage() param #1 'addonId' must be a string or a table, got: " .. type(
				addonId
			) .. "."
		)
	end
...
function DF.Language.RegisterLanguage(
addonId,
	languageId,
	bNotSupportedWoWLanguage,
	languageName,
	languageFont --~RegisterLanguage
)
	if not isValid_AddonID(addonId) then
		error(
			functionCallPath["RegisterLanguage"] .. ": " .. format(
				errorText["AddonID"],
				1
	...
	elseif bNotSupportedWoWLanguage then
		if not languageName then
			error(
				functionCallPath["RegisterLanguage"] .. ": " .. "Invalid Language Name" .. ", use: " .. functionSignature["RegisterLanguage"] .. "."
			)
		end
		if not languageFont then
			error(
				functionCallPath["RegisterLanguage"] .. ": " .. "Invalid Language Font" .. ", use: " .. functionSignature["RegisterLanguage"] .. "."
			)
		end

		if not supportedGameLanguages[languageId] then
			languagesAvailable[languageId] = {
				text = languageName,
				font = languageFont,
			}
		end
...
function DF.Language.SetOption(addonId, optionId, value)
	if not isValid_AddonID(addonId) then
		error(
			functionCallPath["SetOption"] .. ": " .. format(
				errorText["AddonID"],
				1
			) .. ", use: " .. functionSignature["SetOption"] .. "."
		)
	end
	...
	if not addonNamespaceTable then
		error(
			functionCallPath["SetOption"] .. ": " .. errorText["NoLanguages"] .. ", use: " .. functionSignature["RegisterLanguage"] .. "."
		)
	end
...
function DF.Language.SetFontForLanguageId(addonId, languageId, fontPath)
	if not isValid_AddonID(addonId) then
		error(
			functionCallPath["SetFontForLanguageId"] .. ": " .. format(
				errorText["AddonID"],
				1
	...
	if not addonNamespaceTable then
		error(
			functionCallPath["SetFontForLanguageId"] .. ": " .. errorText["NoLanguages"] .. ", use: " .. functionSignature["RegisterLanguage"] .. "."
		)
	end
...
function DF.Language.SetFontByAlphabetOrRegion(addonId, latin, cyrillic, china, korean, taiwan)
	if not isValid_AddonID(addonId) then
		error(
			functionCallPath["SetFontByAlphabetOrRegion"] .. ": " .. format(
				errorText["AddonID"],
				1
	...
	if not addonNamespaceTable then
		error(
			functionCallPath["SetFontByAlphabetOrRegion"] .. ": " .. errorText["NoLanguages"] .. ", use: " .. functionSignature["RegisterLanguage"] .. "."
		)
	end
...
function DF.Language.GetLanguageTable(addonId, languageId)
	if not isValid_AddonID(addonId) then
		error(
			functionCallPath["GetLanguageTable"] .. ": " .. format(
				errorText["AddonID"],
				1
			) .. ", use: " .. functionSignature["GetLanguageTable"] .. "."
		)
	end
	...
	if not addonNamespaceTable then
		error(
			functionCallPath["GetLanguageTable"] .. ": " .. errorText["NoLanguages"] .. ", use: " .. functionSignature["RegisterLanguage"] .. "."
		)
	end
	...
	if not languageTable then
		error(
			functionCallPath["GetLanguageTable"] .. ": " .. errorText["LanguageIDNotRegistered"] .. ", use: " .. functionSignature["RegisterLanguage"] .. "."
		)
	end
...
function DF.Language.SetCurrentLanguage(addonId, newLanguageId)
	if not isValid_AddonID(addonId) then
		error(
			functionCallPath["SetCurrentLanguage"] .. ": " .. format(
				errorText["AddonID"],
				1
			) .. ", use: " .. functionSignature["SetCurrentLanguage"] .. "."
		)
	end
	...
	if not addonNamespaceTable then
		error(
			functionCallPath["SetCurrentLanguage"] .. ": " .. errorText["NoLanguages"] .. ", use: " .. functionSignature["RegisterLanguage"] .. "."
		)
	end
	...
	if not languageTable then
		error(
			functionCallPath["SetCurrentLanguage"] .. ": " .. errorText["LanguageIDNotRegistered"] .. ", use: " .. functionSignature["RegisterLanguage"] .. "."
		)
	end
...
function DF.Language.GetText(addonId, phraseId, silent)
	if not isValid_AddonID(addonId) then
		error(
			functionCallPath["GetText"] .. ": " .. format(
				errorText["AddonID"],
				1
			) .. ", use: " .. functionSignature["GetText"] .. "."
		)
	...
	if not addonNamespaceTable then
		error(
			functionCallPath["GetText"] .. ": " .. errorText["NoLanguages"] .. ", use: " .. functionSignature["RegisterLanguage"] .. "."
		)
	end
	...
	if not silent then
		error(
			functionCallPath["GetText"] .. ": " .. errorText["PhraseIDNotRegistered"] .. ", use: " .. functionSignature["GetLanguageTable"] .. "['PhraseID'] = 'translated text'."
		)
	end
...
function DF.Language.RegisterObject(addonId, object, phraseId, silent, ...)
	if not isValid_AddonID(addonId) then
		error(
			functionCallPath["RegisterObject"] .. ": " .. format(
				errorText["AddonID"],
				1
			) .. ", use: " .. functionSignature["RegisterObject"] .. "."
		)
	end
	...
	if not addonNamespaceTable then
		error(
			functionCallPath["RegisterObject"] .. ": " .. errorText["NoLanguages"] .. ", use: " .. functionSignature["RegisterLanguage"] .. "."
		)
	end
	...
	if not isValid_Text(text) then
		if not silent then
			error(
				functionCallPath["RegisterObject"] .. ": " .. errorText["PhraseIDNotRegistered"] .. ", use: " .. functionSignature["GetLanguageTable"] .. "['PhraseID'] = 'translated text'."
			)
		else
			text = phraseId
		end
...
function DF.Language.UpdateObjectArguments(addonId, object, ...)
	if not isValid_AddonID(addonId) then
		error(
			functionCallPath["UpdateObjectArguments"] .. ": " .. format(
				errorText["AddonID"],
				1
	...
	if not addonNamespaceTable then
		error(
			functionCallPath["UpdateObjectArguments"] .. ": " .. errorText["NoLanguages"] .. ", use: " .. functionSignature["RegisterLanguage"] .. "."
		)
	end
	...
	if not phraseInfoTable then
		error(
			functionCallPath["UpdateObjectArguments"] .. ": " .. errorText["ObjectNotRegistered"] .. ", use: " .. functionSignature["RegisterObject"] .. "."
		)
	end
	updatePhraseInfo_Arguments(phraseInfoTable, ...)
...
--@silent: if true won't error on invalid phrase text or table already registered, it will still error on invalid addonId, table, key and phraseId
--@vararg: arguments to pass for format(text, ...)
function DF.Language.RegisterTableKey(
addonId,
	table,
	key,
	phraseId,
	...
	if not isValid_AddonID(addonId) then
		error(
			functionCallPath["RegisterTableKey"] .. ": " .. format(
				errorText["AddonID"],
				1
			) .. ", use: " .. functionSignature["RegisterTableKey"] .. "."
		)
	end
	...
	if not addonNamespaceTable then
		error(
			functionCallPath["RegisterTableKey"] .. ": " .. errorText["NoLanguages"] .. ", use: " .. functionSignature["RegisterLanguage"] .. "."
		)
	end
	...
	if not isValid_Text(text) then
		if not silent then
			error(
				functionCallPath["RegisterTableKey"] .. ": " .. errorText["PhraseIDNotRegistered"] .. ": " .. phraseId .. ", use: " .. functionSignature["GetLanguageTable"] .. "['PhraseID'] = 'translated text'."
			)
		else
			text = phraseId
		end
...
function DF.Language.UpdateTableKeyArguments(addonId, table, key, ...)
	if not isValid_AddonID(addonId) then
		error(
			functionCallPath["UpdateTableKeyArguments"] .. ": " .. format(
				errorText["AddonID"],
				1
	...
	if not addonNamespaceTable then
		error(
			functionCallPath["UpdateTableKeyArguments"] .. ": " .. errorText["NoLanguages"] .. ", use: " .. functionSignature["RegisterLanguage"] .. "."
		)
	end
	...
	if not isTableKeyRegistered(addonNamespaceTable, table) then
		error(
			functionCallPath["UpdateTableKeyArguments"] .. ": " .. errorText["TableKeyNotRegistered"] .. ", use: " .. functionSignature["RegisterTableKey"] .. "."
		)
	end
	...
	if not isKeyRegisteredInTableKey(tableKeyTable, key) then
		error(
			functionCallPath["UpdateTableKeyArguments"] .. ": " .. errorText["KeyNotRegistered"] .. ", use: " .. functionSignature["RegisterTableKey"] .. "."
		)
	end
...
function DF.Language.RegisterTableKeyWithDefault(
addonId,
	table,
	key,
	phraseId,
...
function DF.Language.RegisterObjectWithDefault(addonId, object, phraseId, defaultText, ...)
	if not isValid_Object(object) then
		error(
			functionCallPath["RegisterObjectWithDefault"] .. ": " .. format(
				errorText["InvalidObject"],
				2
...
function DF.Language.CreateLocTable(addonId, phraseId, shouldRegister, silent, ...)
	if not isValid_AddonID(addonId) then
		error(
			functionCallPath["CreateLocTable"] .. ": " .. format(
				errorText["AddonID"],
				1
			) .. ", use: " .. functionSignature["CreateLocTable"] .. "."
		)
	end
	...
	if not addonNamespaceTable then
		error(
			functionCallPath["CreateLocTable"] .. ": " .. errorText["NoLanguages"] .. ", use: " .. functionSignature["CreateLocTable"] .. "."
		)
	end
	...
	if not text then
		if not silent then
			error(
				functionCallPath["CreateLocTable"] .. ": " .. errorText["PhraseIDNotRegistered"] .. ", use: " .. functionSignature["CreateLocTable"] .. "."
			)
		end
		return
	end
...
function DF.Language.CanRegisterLocTable(locTable)
	if not DF.Language.IsLocTable(locTable) then
		error(
			functionCallPath["CanRegisterLocTable"] .. ": " .. format(
				errorText["InvalidLocTable"],
				1
...
function DF.Language.UnpackLocTable(locTable)
	if (type(locTable) ~= "table") then
		error(
			functionCallPath["UnpackLocTable"] .. ": " .. format(
				errorText["InvalidLocTable"],
				1
			) .. ", use: " .. functionSignature["UnpackLocTable"] .. "."
		)
	end
...
function DF.Language.RegisterTableKeyWithLocTable(table, key, locTable, silence)
	if not DF.Language.IsLocTable(locTable) then
		error(
			functionCallPath["RegisterTableKeyWithLocTable"] .. ": " .. format(
				errorText["InvalidLocTable"],
				3
	...
	if not shouldRegister then
		error(
			functionCallPath["RegisterTableKeyWithLocTable"] .. ": " .. errorText["LocTableCantRegister"] .. ", use: " .. functionSignature["RegisterTableKeyWithLocTable"] .. "."
		)
	end
...
function DF.Language.RegisterObjectWithLocTable(object, locTable, silence)
	if not isValid_Object(object) then
		error(
			functionCallPath["RegisterObjectWithLocTable"] .. ": " .. format(
				errorText["InvalidObject"],
				1
	...
	if not shouldRegister then
		error(
			functionCallPath["RegisterObjectWithLocTable"] .. ": " .. errorText["LocTableCantRegister"] .. ", use: " .. functionSignature["RegisterObjectWithLocTable"] .. "."
		)
	end
...
function DF.Language.SetTextWithLocTable(object, locTable)
	if not isValid_Object(object) then
		error(
			functionCallPath["SetTextWithLocTable"] .. ": " .. format(
				errorText["InvalidObject"],
				1
	...
	if not addonNamespaceTable then
		error(
			functionCallPath["SetTextWithLocTable"] .. ": " .. errorText["AddonIDInvalidOrNotRegistered"] .. ", use: " .. functionSignature["RegisterLanguage"] .. "."
		)
	end
...
function DF.Language.SetTextWithLocTableWithDefault(object, locTable, defaultText)
	if not isValid_Object(object) then
		error(
			functionCallPath["SetTextWithLocTableWithDefault"] .. ": " .. format(
				errorText["InvalidObject"],
				1
...
function DF.Language.SetTextIfLocTableOrDefault(object, locTable)
	if not isValid_Object(object) then
		error(
			functionCallPath["SetTextIfLocTableOrDefault"] .. ": " .. format(
				errorText["InvalidObject"],
				1
...
function DF.Language.CreateLanguageSelector(addonId, parent, callback, selectedLanguage)
	if not isValid_AddonID(addonId) then
		error(
			functionCallPath["CreateLanguageSelector"] .. ": " .. format(
				errorText["AddonID"],
				1
	...
	if not addonNamespaceTable then
		error(
			functionCallPath["CreateLanguageSelector"] .. ": " .. errorText["AddonIDInvalidOrNotRegistered"] .. ", use: " .. functionSignature["RegisterLanguage"] .. "."
		)
	end
