...
function DF:CreateAuraConfigPanel(parent, name, db, changeCallback, options, texts)
	...
	local automaticTrackingCheckbox =
		DF:CreateSwitch(
			methodSelectionBackground,
			onSwitchTrackingMethod,
			newAuraPanel.db.aura_tracker.track_method == 0x1,
			nil,
			nil,
			nil,
			nil,
	...
	local manualTrackingCheckbox =
		DF:CreateSwitch(
			methodSelectionBackground,
			onSwitchTrackingMethod,
			newAuraPanel.db.aura_tracker.track_method == 0x2,
			nil,
			nil,
			nil,
			nil,
	...
	--black list
	--create labels
	local buffBlacklistLabel =
		self:CreateLabel(
			blacklistAddBackground,
			texts.MANUAL_ADD_BLACKLIST_BUFF,
			DF:GetTemplate("font", "OPTIONS_FONT_TEMPLATE")
		)
	local debuffBlacklistLabel =
		self:CreateLabel(
			blacklistAddBackground,
			texts.MANUAL_ADD_BLACKLIST_DEBUFF,
			DF:GetTemplate("font", "OPTIONS_FONT_TEMPLATE")
		)

	local buffNameBlacklistEntry =
		self:CreateTextEntry(
			blacklistAddBackground,
			function() end,
			textEntryWidth,
			20,
			"AddBuffBlacklistTextBox",
			_,
			_,
	...
	local debuffNameBlacklistEntry =
		self:CreateTextEntry(
			blacklistAddBackground,
			function() end,
			textEntryWidth,
			20,
			"AddDebuffBlacklistTextBox",
			_,
			_,
			options_dropdown_template
		)
	...
	local addBuffNameToBacklistButton = self:CreateButton(
		blacklistAddBackground,
		function()
				...
				if text:find(";") then
					for _, spellName in ipairs({strsplit(";", text)}) do
						...
						if spellId then
							...
							DetailsFramework.Msg(
								{__name = "DetailsFramework"},
								"Spell not found: " .. (spellName or "")
							)
						end
					end
				else
	...
	local addBuffIDToBacklistButton = self:CreateButton(
		blacklistAddBackground,
		function()
			local text = buffNameBlacklistEntry.text
			buffNameBlacklistEntry:SetText("")
	...
	local addDebuffNameToBacklistButton = self:CreateButton(
		blacklistAddBackground,
		function()
	...
	local addDebuffIDToBacklistButton = self:CreateButton(
		blacklistAddBackground,
		function()
			local text = debuffNameBlacklistEntry.text
			debuffNameBlacklistEntry:SetText("")
	...
	--track list
	local buffTracklistLabel =
		self:CreateLabel(
			tracklistAddBackground,
			texts.MANUAL_ADD_TRACKLIST_BUFF,
			DF:GetTemplate("font", "OPTIONS_FONT_TEMPLATE")
		)
	local debuffTracklistLabel =
		self:CreateLabel(
			tracklistAddBackground,
			texts.MANUAL_ADD_TRACKLIST_DEBUFF,
			DF:GetTemplate("font", "OPTIONS_FONT_TEMPLATE")
		)

	local buffNameTracklistEntry =
		self:CreateTextEntry(
			tracklistAddBackground,
			function() end,
			textEntryWidth,
	...
	local debuffNameTracklistEntry =
		self:CreateTextEntry(
			tracklistAddBackground,
			function() end,
			textEntryWidth,
			20,
			"AddDebuffTracklistTextBox",
			_,
			_,
			options_dropdown_template
		)
	...
	local addDebuffNameToTracklistButton = self:CreateButton(
		tracklistAddBackground,
		function()
	...
	local addDebuffIDToTracklistButton = self:CreateButton(
		tracklistAddBackground,
		function()
			local text = debuffNameTracklistEntry.text
			debuffNameTracklistEntry:SetText("")
	...
	local addBuffNameToTracklistButton = self:CreateButton(
		tracklistAddBackground,
		function()
	...
	local addBuffIDToTracklistButton = self:CreateButton(
		tracklistAddBackground,
		function()
			local text = buffNameTracklistEntry.text
			buffNameTracklistEntry:SetText("")
	...
	local debuffTrackedAuraScrollBox =
		detailsFramework:CreateAuraScrollBox(
			auraPanel_Auto,
			"$parentDebuffTracked",
			newAuraPanel.db.aura_tracker.debuff_tracked,
			onAuraRemoveCallback,
			options
		)
	...
	local buffIgnoredAuraScrollBox =
		detailsFramework:CreateAuraScrollBox(
			auraPanel_Auto,
			"$parentBuffIgnored",
			newAuraPanel.db.aura_tracker.buff_banned,
			onAuraRemoveCallback,
			options
		)
	...
	local debuffIgnoredAuraScrollBox =
		detailsFramework:CreateAuraScrollBox(
			auraPanel_Auto,
			"$parentDebuffIgnored",
			newAuraPanel.db.aura_tracker.debuff_banned,
			onAuraRemoveCallback,
			options
		)
	...
	local buffTrackedAuraScrollBox =
		detailsFramework:CreateAuraScrollBox(
			auraPanel_Auto,
			"$parentBuffTracked",
	...
	local buffs_added =
		self:CreateScrollBox(
			...
			"$parentBuffsAdded",
			scroll_refresh,
			newAuraPanel.db.aura_tracker.buff,
			scroll_width,
			scroll_height,
			scroll_lines,
			scroll_line_height
	...
	local debuffs_added =
		self:CreateScrollBox(
			...
			"$parentDebuffsAdded",
			scroll_refresh,
			newAuraPanel.db.aura_tracker.debuff,
			scroll_width,
			scroll_height,
			scroll_lines,
			scroll_line_height
	...
	new_debuff_entry.tooltip =
		"Enter the debuff name using lower case letters.\n\nYou can add several spells at once using |cFFFFFF00;|r to separate each spell name."

	new_buff_entry:SetJustifyH("left")
	new_debuff_entry:SetJustifyH("left")

	local add_buff_button = self:CreateButton(
		auraPanel_Manual,
		function()
	...
	local add_debuff_button = self:CreateButton(
		auraPanel_Manual,
		function()
	...
	local multiple_spells_label =
		DF:CreateLabel(
			buffs_added,
			"You can add multiple auras at once by separating them with ';'.\nExample: Fireball; Frostbolt; Flamestrike",
			10,
			"gray"
		)
	...
	local export_buff_button = self:CreateButton(
		auraPanel_Manual,
		function()
			local str = ""
	...
	local export_debuff_button = self:CreateButton(
		auraPanel_Manual,
		function()
			local str = ""
