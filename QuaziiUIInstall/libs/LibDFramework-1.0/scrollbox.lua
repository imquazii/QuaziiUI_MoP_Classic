	...
	SetData = function(self, data)
		...
		if self.OnSetData then
			detailsFramework:CoreDispatch(
				(self:GetName() or "ScrollBox") .. ":OnSetData()",
				self.OnSetData,
				self,
				self.data
			)
...
function detailsFramework:CreateGridScrollBox(parent, name, refreshFunc, data, createColumnFrameFunc, options)
	...
	local lineAmount =
		type(options.line_amount) == "number" and options.line_amount or grid_scrollbox_options.line_amount
	---@cast lineAmount number

	local lineHeight =
		type(options.line_height) == "number" and options.line_height or grid_scrollbox_options.line_height
...
function detailsFramework:CreateMenuWithGridScrollBox(
parent,
	name,
	refreshMeFunc,
	refreshButtonFunc,
	...
	gridScrollBox =
		detailsFramework:CreateGridScrollBox(parent, name, refreshLine, {}, createButton, gridScrollBoxOptions)
...
function detailsFramework:CreateAuraScrollBox(parent, name, data, onAuraRemoveCallback, options, onSetupAuraClick)
	...
	local onEnterAuraLine = function(line)
		...
		if line.setupbutton:IsShown() then  end
		...
		if bTrackByName then --the user entered the spell name to track the spell
			...
			--the user entered the spellId to track the spell
			GameCooltip:Preset(2)
			GameCooltip:SetOwner(line, "left", "right", 2, 0)
			GameCooltip:SetOption("TextSize", 10)
...
function detailsFramework:CreateCanvasScrollBox(parent, child, name, options)
	...
	local canvasScrollBox =
		CreateFrame(
			"scrollframe",
			name or ("DetailsFrameworkCanvasScroll" .. math.random(50000, 10000000)),
			parent,
			"BackdropTemplate, UIPanelScrollFrameTemplate"
		)
