...
local attributes = {
	...
	Texture = {{
		...
		setter = function(widget, value)
			detailsFramework:SetAnchor(widget, value, widget:GetParent())
		end,
	}, {
		key = "anchoroffsetx",
	...
	FontString = {{
		...
		maxvalue = 120,
		setter = function(widget, value)
			widget:SetFont(widget:GetFont(), value, select(3, widget:GetFont()))
		end,
	}, {
		key = "font",
		label = "Font",
		...
		widget = "color",
		setter = function(widget, value)
			widget:SetTextColor(unpack(value))
		end,
	}, {
		key = "alpha",
		label = "Alpha",
		...
		widget = "toggle",
		setter = function(widget, value)
			widget:SetShadowColor(
				widget:GetShadowColor(),
				select(2, widget:GetShadowColor()),
				select(3, widget:GetShadowColor()),
				value and 0.5 or 0
			)
		end,
	}, {
		...
		widget = "color",
		setter = function(widget, value)
			widget:SetShadowColor(unpack(value))
		end,
	}, {
		key = "shadowoffsetx",
		...
		maxvalue = 10,
		setter = function(widget, value)
			widget:SetShadowOffset(value, select(2, widget:GetShadowOffset()))
		end,
	}, {
		key = "shadowoffsety",
		...
		maxvalue = 10,
		setter = function(widget, value)
			widget:SetShadowOffset(widget:GetShadowOffset(), value)
		end,
	}, {
		key = "outline",
		label = "Outline",
		...
		setter = function(widget, value)
			widget:SetFont(widget:GetFont(), select(2, widget:GetFont()), value)
		end,
	}, {widget = "blank"}, {
		key = "anchor",
		label = "Anchor",
...
detailsFramework.EditorMixin = {
		...
		editorFrame.AnchorFrames = {
			...
			SetupAnchorsForObject = function(self, anchorTable)
				...
				if registeredObject.refFrame then
					...
					local anchorFrameSelected =
						self:GetAnchorFrame(detailsFramework.InsidePointsToAnchor[sideSelected] or sideSelected)
			...
			CreateNineAnchors = function(self)
				...
				for i = 1, 9 do
					local anchorFrame =
						CreateFrame("button", "$parentAnchorFrame" .. i, overTheTopFrame, "BackdropTemplate")
			...
			Setup = function(self, object, registeredObject, onTickWhileMoving, onTickNotMoving)
				for i = 1, amountOfMovers do
					...
					moverFrame:SetScript("OnMouseDown", function()
						...
						editorFrame.currentObjectNinePoints =
							detailsFramework.Math.GetNinePoints(registeredObject.refFrame)
					...
					moverFrame:SetScript("OnMouseUp", function()
						...
						local startX, startY = moverFrame:GetCenter()
						local closestPoint =
							editorFrame.currentObjectNinePoints:GetClosestPoint(CreateVector2D(startX, startY))
	...
	PrepareObjectForEditing = function(
	self --~edit
	)
		...
		if (not object or not profileTable) then return end
		...
		if (extraOptions and #extraOptions > 0) then
			...
			attributeListWithExtraOptions[#attributeListWithExtraOptions + 1] = {
				widget = "blank",
				default = true,
			}
		...
		local options_text_template =
			self.options.text_template or detailsFramework:GetTemplate("font", "OPTIONS_FONT_TEMPLATE")
	...
	UpdateProfileTableOnAllRegisteredObjects = function(self, profileTable)
		assert(
			...
			"UpdateProfileTableOnAllRegisteredObjects() expects a table on #1 parameter."
		)
	...
	self,
		...
		assert(
			type(callback) == "function" or callback == nil,
			"RegisterObjectToEdit() expects a function or nil as the #7 parameter."
		)
	...
	CreateObjectSelectionList = function(self, scroll_width, scroll_height, scroll_lines, scroll_line_height)
		...
		local createLineFunc = function(
		self,
			index -- ~createline --~line
		)
		...
		local selectObjectScrollBox =
			detailsFramework:CreateScrollBox(
				self:GetParent(),
				"$parentSelectObjectScrollBox",
				refreshFunc,
				editorFrame:GetAllRegisteredObjects(),
				scroll_width,
				scroll_height,
				scroll_lines,
				scroll_line_height
...
function detailsFramework:CreateEditor(parent, name, options)
	...
	local canvasFrame =
		detailsFramework:CreateCanvasScrollBox(
			editorFrame,
			optionsFrame,
			name .. "CanvasScrollBox",
			canvasScrollBoxOptions
		)
	...
	if editorFrame.options.create_object_list then
		...
		local objectSelector =
			editorFrame:CreateObjectSelectionList(scrollWidth, scrollHeight, scrollLinesAmount, scrollLineHeight)
