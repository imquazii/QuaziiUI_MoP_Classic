...
TextEntryMetaFunctions.__call = function(object, value) end
...
local OnEnterPressed = function(textentry, byScript)
	...
	if object.ignoreNextCallback then
		detailsFramework.Schedules.RunNextTick(function()
			object.ignoreNextCallback = nil
		end)
		return
	end
	...
	if (string.len(text) > 0) then
		...
		if textentry.MyObject.func then
			...
			textentry.MyObject.func(
				textentry.MyObject.param1,
				textentry.MyObject.param2,
				text,
				textentry,
				byScript or textentry
			)
		end
	else
		textentry:SetText("")
...
function TextEntryMetaFunctions:SetAsSearchBox()
	if self.__bIsSearchBox then return end
...
---@return df_textentry
function detailsFramework:CreateTextEntry(
parent,
	textChangedCallback,
	width,
	height,
	...
	return detailsFramework:NewTextEntry(
		parent,
		parent,
		name,
		member,
...
function detailsFramework:NewTextEntry(
parent,
	container,
	name,
	member,
	...
	newTextEntryObject.editbox:SetBackdrop({
	...
	--editbox label
	newTextEntryObject.editbox.label =
		newTextEntryObject.editbox:CreateFontString("$parent_Desc", "OVERLAY", "GameFontHighlightSmall")
	...
	if withLabel then
		local label =
			detailsFramework:CreateLabel(newTextEntryObject.editbox, withLabel, nil, nil, nil, "label", nil, "overlay")
...
local AutoComplete_OnChar = function(editboxWidget, char, capsule)
	...
	if editboxWidget.ignore_input then return end
	...
	if (capsule.lastword:len() >= 2) then
		...
		if capsule.ShouldOptimizeAutoComplete then
			...
			if wordList then
				for i = 1, #wordList do
					local thisWord = wordList[i]
					if (thisWord and (thisWord:find("^" .. capsule.lastword) or thisWord:lower():find(
						"^" .. capsule.lastword
					))) then
...
---@return df_luaeditor
function detailsFramework:NewSpecialLuaEditorEntry(
parent,
	width,
	height,
	member,
	...
	local scrollframeNumberLines =
		CreateFrame(
			"ScrollFrame",
			name and (name .. "NumberLines"),
			borderframe,
	...
	if showLineNumbers then
		scrollframeNumberLines.editbox =
			CreateFrame("editbox", name and "$parentLineNumbers", scrollframeNumberLines, "BackdropTemplate")
		...
		scrollframe.editbox:HookScript("OnTextChanged", function()
			if currentUpdateLineCounterTimer then return end
