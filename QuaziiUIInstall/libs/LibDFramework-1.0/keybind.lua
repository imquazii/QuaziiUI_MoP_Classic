...
local defaultMouseKeybinds = {{
	...
	action = "togglemenu",
	name = _G["SLASH_TEXTTOSPEECH_MENU"],
	keybind = "type2",
	...
	name = _G["FOCUS"],
	keybind = "type3",
	icon = [[Interface\MINIMAP\TRACKING\Focus]],
...
local DARK_BUTTON_TEMPLATE = detailsFramework:InstallTemplate(
	"button",
	"DARK_BUTTON_TEMPLATE",
	{
		backdropcolor = {.1, .1, .1, .98},
	},
...
detailsFramework.KeybindMixin = {
	...
	DeleteMacro = function(self)
		...
		if not bIsEditingKeybind then return end

		if not self:IsKeybindActionMacro(keybindTable.action) then return end
	...
	OnUserPressedKeybind = function(
	self,
		keyPressed --called from OnUserClickedToChooseKeybind and from OnKeyDown() script
	)
		--if the player presses a control key, ignore it
		if ignoredKeys[keyPressed] then return end
	...
	self,
		...
		for i = 1, totalLines do
			...
			if keybindScrollData then
				...
				local actionName, iconTexture, actionId, keybindTable, bIsAvailable, sortNumber, actionIdentifier =
					unpack(keybindScrollData)
				...
				if (actionName == "@separator") then
					...
					line.setKeybindButton:SetClickFunction(
						keyBindFrame.OnUserClickedToChooseKeybind,
						actionIdentifier,
						keybindTable,
						"left"
					)
					...
					if (keydindText ~= "") then
						line.clearKeybindButton:Enable()
						line.clearKeybindButton:SetClickFunction(
							keyBindFrame.ClearKeybind,
							actionIdentifier,
							keybindTable
						)
						line.editKeybindSettingsButton:Enable()
						line.editKeybindSettingsButton:SetClickFunction(
							keyBindFrame.StartEditingKeybindSettings,
							actionIdentifier,
							keybindTable
						)
					else
	...
	OnEnterScrollLine = function(self)
		...
		if self.bIsSeparator then return end
			...
			if keybindScrollData then
				...
				if (actionName ~= "@separator" and keybindTable) then
					...
					if not bCanLoad then
						editPanel.conditionsFailLoadReasonText:SetText(
							"This keybind can't be loaded because it's conditions are not met:\n- " .. (reason or "")
						)
					else
						editPanel.conditionsFailLoadReasonText:SetText("")
					end
	...
	---@param index number
	CreateKeybindScrollLine = function(
	keybindScroll,
		index --~create
	)
		...
		---@type df_button
		line.setKeybindButton =
			detailsFramework:CreateButton(
				line,
				function() end,
				headerTable[3].width,
				keyBindFrame.options.line_height - 6,
				"",
				nil,
				nil,
				nil,
				"SetNewKeybindButton",
				"$parentSetNewKeybindButton",
				0,
				nil,
		...
		line.clearKeybindButton =
			detailsFramework:CreateButton(
				...
				nil,
				"DeleteKeybindButton",
				"$parentDeleteKeybindButton",
				2,
				nil,
				options_text_template
			)
		line.clearKeybindButton:SetBackdropBorderColor(0, 0, 0, 0)
		line.clearKeybindButton:SetIcon(
			[[Interface\COMMON\CommonIcons]],
			nil,
			nil,
			nil,
			{0.1264, 0.2514, 0.5048, 0.7548},
			nil,
			nil,
			4
		)
		...
		---@type df_button
		line.editKeybindSettingsButton =
			detailsFramework:CreateButton(
				line,
				keyBindFrame.StartEditingKeybindSettings,
				16,
	...
	self --~scroll
		...
		local onClickCreateMacroButton = function()
			--~macro
		...
		local createMacroButton =
			detailsFramework:CreateButton(
				self.Header,
				onClickCreateMacroButton,
				200,
				32,
				"Create Macro Keybind",
				nil,
				nil,
				nil,
				"CreateMacroButton",
				"$parentCreateMacroButton",
				0,
				DARK_BUTTON_TEMPLATE,
				detailsFramework:GetTemplate("font", "OPTIONS_FONT_TEMPLATE")
			)
		...
		local keybindScroll =
			detailsFramework:CreateScrollBox(
				self,
				"$parentScrollBox",
				detailsFramework.KeybindMixin.RefreshKeybindScroll,
				{},
				scroll_width,
				scroll_height,
				scroll_lines,
		...
		function keybindScroll.UpdateScroll() --~update
			...
			if self.options.show_unitcontrols then
				for i, mouseActionKeyInfo in ipairs(defaultMouseKeybinds) do
					...
					local thisScrollData =
						{
							keybindData and keybindData.name or mouseActionName,
							keybindData and keybindData.icon or mouseIcon,
							mouseActionId,
							keybindData or false,
							true,
							0,
							actionIdentifier,
						}
			...
			if self.options.show_macros then
				...
				table.sort(allKeybindMacros, function(t1, t2)
				...
				for i, keybindData in ipairs(allKeybindMacros) do
					...
					---@type keybind_scroll_data
					local thisScrollData =
						{macroName, macroIcon, actionId, keybindData, bCanLoad, sortScore, actionIdentifier}
					if bCanLoad then
			...
			if self.options.show_spells then
				...
				for spellId, bIsAvailable in pairs(allPlayerSpells) do
					...
					if spellName then
						...
						local thisScrollData =
							{
								keybindData and keybindData.name or spellName,
								keybindData and keybindData.icon or spellIcon,
								actionId,
								keybindData or false,
								bIsAvailable,
								sortScore,
				...
				table.sort(spellsNotAvailable, function(a, b)
					return a[6] < b[6]
				end)
			end
	...
	StartEditingKeybindSettings = function(self, button, actionIdentifier, keybindTable)
		...
		local bIsListening = keyBindFrame:GetListeningState()
		if bIsListening then return end
	...
	self --~edit
		...
		local iconPickerButton = detailsFramework:CreateButton(
			editFrame,
			function()
				detailsFramework:IconPick(iconPickerButtonCallback, true)
			end,
			20,
			20,
			"",
		...
		local editMacroEditBox =
			detailsFramework:NewSpecialLuaEditorEntry(
				editFrame,
				self.options.edit_width - 35,
				200,
				"editMacroEditBox",
				"$parentEditMacroEditBox",
				true
			)
		editMacroEditBox:SetBackdrop({
		...
		local openConditionsPanel = function()
			...
			detailsFramework:OpenLoadConditionsPanel(conditionsSettings, onLoadConditionsChange, {
				title = "Keybind Load Conditions",
				name = editFrame.keybindTable.name,
			})
		end

		local conditionsButton =
			detailsFramework:CreateButton(
				editFrame,
				openConditionsPanel,
				160,
				20,
				"Edit Load Conditions",
				nil,
				nil,
				[[]],
				"conditionsButton",
				"$parentConditionsButton",
				0,
				DARK_BUTTON_TEMPLATE,
				detailsFramework:GetTemplate("font", "OPTIONS_FONT_TEMPLATE")
			)
		...
		local conditionsFailLoadReasonText =
			editFrame:CreateFontString("$parentConditionsFailLoadText", "overlay", "GameFontNormal")
		...
		local deleteMacroButton =
			detailsFramework:CreateButton(
				editFrame,
				onClickDeleteMacroButton,
				160,
				20,
				"Delete This Macro",
				nil,
