...
local CreateImageEditorFrame = function()
	...
	local background =
		DF:NewImage(editorWindow, nil, nil, nil, "background", nil, "background", "$parentBackgroundImage")
	...
	topSlider:SetHook("OnEnter", function()
		return true
	end)
	topSlider:SetHook("OnLeave", function()
		return true
	end)
	...
	local bottomSlider =
		DF:NewSlider(editorWindow, nil, "$parentBottomSlider", "bottomSlider", 100, 100, 0.1, 100, 0.1, 100)
	...
	bottomSlider:SetHook("OnEnter", function()
		return true
	end)
	bottomSlider:SetHook("OnLeave", function()
		return true
	end)
	...
	leftSlider:SetHook("OnEnter", function()
		return true
	end)
	leftSlider:SetHook("OnLeave", function()
		return true
	end)
	...
	local rightSlider =
		DF:NewSlider(editorWindow, nil, "$parentRightSlider", "rightSlider", 100, 100, 0.1, 100, 0.1, 100)
	rightSlider:SetAllPoints(editorWindow.widget)
	rightSlider.backdrop = nil
	rightSlider.fractional = true
	rightSlider:SetHook("OnEnter", function()
		return true
	end)
	rightSlider:SetHook("OnLeave", function()
		return true
	end)
	--[
	...
	local buttonsBackground =
		DF:NewPanel(background_frame, nil, "DetailsFrameworkImageEditButtonsBg", nil, 115, 230, false)
	...
	local rightTexCoordButton =
		DF:NewButton(
			buttonsBackground,
			nil,
			"$parentRightTexButton",
			nil,
			100,
			20,
			enableTexEdit,
	...
	local bottomTexCoordButton =
		DF:NewButton(
			buttonsBackground,
			nil,
			"$parentBottomTexButton",
			nil,
			100,
			20,
			enableTexEdit,
	...
	local Alpha =
		DF:NewButton(buttonsBackground, nil, "$parentBottomAlphaButton", nil, 100, 20, alpha, nil, nil, nil, "Alpha", 1)
	...
	local changeColorButton =
		DF:NewButton(
			buttonsBackground,
			nil,
			"$parentOverlayColorButton",
			nil,
			100,
			20,
			changeColor,
	...
	editorWindow.widget:SetScript("OnSizeChanged", function()
		...
		rightCoordTexture.image:SetWidth(
			math.max((editorWindow.frame:GetWidth() / 100 * math.abs(rightSlider:GetValue() - 100)), 1)
		)
		leftCoordTexture.image:SetWidth(editorWindow.frame:GetWidth() / 100 * leftSlider:GetValue())
		bottomCoordTexture:SetHeight(
			math.max((editorWindow.frame:GetHeight() / 100 * math.abs(bottomSlider:GetValue() - 100)), 1)
		)
	...
	local flipButtonH =
		DF:NewButton(buttonsBackground, nil, "$parentFlipButton", nil, 100, 20, flip, 1, nil, nil, "Flip H", 1)
	...
	dragFrame:SetBackdrop({
		edgeFile = [[Interface\Buttons\WHITE8X8]],
		edgeSize = 1,
		bgFile = [[Interface\Worldmap\UI-QuestBlob-Inside]],
		tileSize = 256,
		tile = true,
	})
	...
	editorWindow.accept = function(self, bottom, keepEditing)
		...
		return editorWindow.callback_func(
			edit_texture.width,
			edit_texture.height,
			{edit_texture:GetVertexColor()},
			edit_texture:GetAlpha(),
			coords,
			editorWindow.extra_param
		)
	end
