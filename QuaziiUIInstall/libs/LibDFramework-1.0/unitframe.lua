...
healthBarMetaFunctions.UpdateHealPrediction = function(self)
	...
	if (not currentHealthMax or currentHealthMax <= 0) then return end
	...
	if self.Settings.ShowHealingPrediction then
		...
		--heal absorbs
		local unitHealAbsorb =
			IS_WOW_PROJECT_MAINLINE and self.displayedUnit and UnitGetTotalHealAbsorbs(self.displayedUnit) or 0
	...
	if (self.Settings.ShowShields and IS_WOW_PROJECT_MAINLINE) then
		...
		if (unitDamageAbsorb > 0) then
			...
			self.shieldAbsorbIndicator:SetWidth(
				max(1, min(width * damageAbsorbPercent, abs(healthPercent - 1) * width))
			)
			...
			if ((healthPercent + damageAbsorbPercent) > 1) then
				...
				if (GetTime() >= self.nextShieldHook) then
					self:RunHooksForWidget(
						"OnAbsorbOverflow",
						self,
						self.displayedUnit,
						healthPercent + damageAbsorbPercent - 1
					)
...
function detailsFramework:CreateHealthBar(parent, name, settingsOverride)
	assert(
		name or parent:GetName(),
		"DetailsFramework:CreateHealthBar parameter 'name' omitted and parent has no name."
	)
...
detailsFramework.CastFrameFunctions = {
	...
	SetDefaultColor = function(self, colorType, r, g, b, a)
		assert(
			...
			"DetailsFramework: CastBar:SetDefaultColor require a string in the first argument."
		)
	...
	OnTick_Casting = function(self, deltaTime)
		...
		if self:CheckCastIsDone() then return
		else
	...
	OnTick = function(self, deltaTime)
		if self.casting then
			if not self:OnTick_Casting(deltaTime) then return end
		...
		elseif self.channeling then
			if not self:OnTick_Channeling(deltaTime) then return end
	...
	Animation_FadeOutStarted = function(self) end,
	...
	--animation start script
	Animation_FadeInStarted = function(self) end,
	--animation finished script
	...
	UpdateCastingInfo = function(self, unit, ...)
		...
		local name, text, texture, startTime, endTime, isTradeSkill, uciCastID, notInterruptible, uciSpellID =
		...
		--is valid?
		if not self:IsValid(unit, name, isTradeSkill, true) then return end
	...
	UpdateChannelInfo = function(self, unit, ...)
		local unitID, castID, spellID = ...
		local name, text, texture, startTime, endTime, isTradeSkill, notInterruptible, uciSpellID, _, numStages =
			CastInfo.UnitChannelInfo(unit)
		spellID = uciSpellID or spellID
		castID = uciCastID or castID
	...
	UNIT_SPELLCAST_DELAYED = function(self, unit, ...)
		...
		if not self:IsValid(unit, name, isTradeSkill) then return end
...
function detailsFramework:CreateCastBar(parent, name, settingsOverride)
	...
	do
		...
		local fadeOutAnimationHub =
			detailsFramework:CreateAnimationHub(
				castBar,
		...
		local fadeInAnimationHub =
			detailsFramework:CreateAnimationHub(
				castBar,
				detailsFramework.CastFrameFunctions.Animation_FadeInStarted,
				detailsFramework.CastFrameFunctions.Animation_FadeInFinished
			)
		...
		local flashAnimationHub = detailsFramework:CreateAnimationHub(
			flashTexture,
			function()
				flashTexture:Show()
			end,
...
detailsFramework.UnitFrameFunctions = {
	...
	UnitFrameEvents = {--run for all units
	{
		"PLAYER_ENTERING_WORLD",
	}, {
		...
		true,
	}, {"UNIT_EXITED_VEHICLE", true}, {"UNIT_PET", true}, {"UNIT_THREAT_LIST_UPDATE", true}},
	...
	UpdateHealthColor = function(self, r, g, b)
		...
		if not self.Settings.CanModifyHealhBarColor then return end
	...
	UpdateName = function(self)
		if not self.Settings.ShowUnitName then return end
...
---@return df_unitframe
function detailsFramework:CreateUnitFrame(
parent,
	name,
	unitFrameSettingsOverride,
	healthBarSettingsOverride,
	castBarSettingsOverride,
