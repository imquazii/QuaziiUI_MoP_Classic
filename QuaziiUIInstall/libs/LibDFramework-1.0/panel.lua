...
if (not detailsFramework or not DetailsFrameworkCanLoad) then return end
...
local GetSpellTabInfo = GetSpellTabInfo or function(tabLine)
	...
	if skillLine then
		return skillLine.name, skillLine.iconID, skillLine.itemIndexOffset, skillLine.numSpellBookItems, skillLine.isGuild, skillLine.offSpecID
	end
end
...
local GetSpellBookItemInfo = GetSpellBookItemInfo or function(...)
	...
	if si then
		return SpellBookItemTypeMap[si.itemType] or "NONE", (si.itemType == Enum.SpellBookItemType.Flyout or si.itemType == Enum.SpellBookItemType.PetAction) and si.actionID or si.spellID or si.actionID, si
	end
end
local GetSpellBookItemTexture = GetSpellBookItemTexture or function(...)
	return C_SpellBook.GetSpellBookItemTexture(...)
end
local GetSpellTexture = GetSpellTexture or function(...)
				...
				if options.anchor_to_child then
					if (i == breakLine) then
						...
						thisFrame:SetPoint(
							anchorPoint,
							latestFrame,
							i == 1 and "topleft" or anchorAt,
							offsetX,
							i == 1 and 0 or offsetY
						)
						latestFrame = thisFrame
					end
				else
...
function PanelMetaFunctions:CreateRightClickLabel(textType, width, height, showCloseText)
	...
	return detailsFramework:NewLabel(
		self,
		_,
		"$parentRightMouseToClose",
		nil,
		"|TInterface\\TUTORIALFRAME\\UI-TUTORIAL-FRAME:" .. width .. ":" .. height .. ":0:1:512:512:8:70:328:409|t " .. text
	)
end
...
function PanelMetaFunctions:SetBackdrop(background, edge, tilesize, edgesize, tile, left, right, top, bottom)
	...
	elseif (type(background) == "table") then
		...
		local currentBackdrop = self.frame:GetBackdrop() or {
			...
			elseif (rowType == "icon") then
				for i = 1, #self.scrollframe.lines do
					...
					icon:SetPoint(
						...
						line,
						"left",
						self._anchors[#self._anchors] + (((row.width or 22) - 22) / 2),
						0
					)
					icon.func = row.func
				end
...
local create_panel_text = function(self, row)
	...
	local text =
		detailsFramework:NewLabel(row, nil, self._name .. "$parentLabel" .. row.text_total, "text" .. row.text_total)
...
local create_panel_checkbox = function(self, row)
	...
	local switch =
		detailsFramework:NewSwitch(row, nil, "$parentCheckBox" .. row.checkbox_total, nil, 20, 20, nil, nil, false)
...
local create_panel_button = function(self, row)
	...
	local button =
		detailsFramework:NewButton(row, nil, "$parentButton" .. row.button_total, "button" .. row.button_total, 120, 20)
...
local create_panel_icon = function(self, row)
	...
	local icon =
		detailsFramework:NewImage(iconbutton, nil, 20, 20, "artwork", nil, "_icon", "$parentIcon" .. row.icon_total)
...
local create_panel_texture = function(self, row)
	...
	local texture =
		detailsFramework:NewImage(
			...
			"artwork",
			nil,
			"_icon" .. row.texture_total,
			"$parentIcon" .. row.texture_total
		)
...
function detailsFramework:NewFillPanel(parent, rows, name, member, w, h, total_lines, fill_row, autowidth, options)
	...
	local scrollframe =
		CreateFrame("scrollframe", name .. "Scroll", panel.widget, "FauxScrollFrameTemplate", "BackdropTemplate")
	scrollframe:SetScript("OnVerticalScroll", function(self, offset)
		FauxScrollFrame_OnVerticalScroll(self, offset, 20, panel.Refresh)
	end)
...
if ((ColorPickerFrame and ColorPickerFrame.SetupColorPickerAndShow) or toc >= 100205) then -- maybe fallback to only check CPF in the future
	...
	local color_pick_func_cancel = function()
		local r, g, b, a =
			...
			ColorPickerFrame.previousValues.a
...
function detailsFramework:IconPick(callback, close_when_select, param1, param2)
	if not detailsFramework.IconPickFrame then
		...
		detailsFramework.IconPickFrame =
			CreateFrame("frame", "DetailsFrameworkIconPickFrame", UIParent, "BackdropTemplate")
		...
		detailsFramework.IconPickFrame:SetBackdrop({
		...
		--serach
		detailsFramework.IconPickFrame.searchLabel =
			detailsFramework:NewLabel(detailsFramework.IconPickFrame, nil, "$parentSearchBoxLabel", nil, "Search:")
		detailsFramework.IconPickFrame.searchLabel:SetPoint(
			"topleft",
			detailsFramework.IconPickFrame,
			"topleft",
			12,
		...
		detailsFramework.IconPickFrame.searchLabel:SetTemplate(
			detailsFramework:GetTemplate("font", "ORANGE_FONT_TEMPLATE")
		)
		detailsFramework.IconPickFrame.searchLabel.fontsize = 12

		detailsFramework.IconPickFrame.search =
			detailsFramework:NewTextEntry(detailsFramework.IconPickFrame, nil, "$parentSearchBox", nil, 140, 20)
		detailsFramework.IconPickFrame.search:SetPoint(
			"left",
			detailsFramework.IconPickFrame.searchLabel,
			"right",
			2,
		...
		detailsFramework.IconPickFrame.search:SetTemplate(
		...
		detailsFramework.IconPickFrame.customIcon =
			detailsFramework:CreateLabel(
				detailsFramework.IconPickFrame,
				"Icon Path:",
				detailsFramework:GetTemplate("font", "ORANGE_FONT_TEMPLATE")
			)
		detailsFramework.IconPickFrame.customIcon:SetPoint(
			"bottomleft",
			detailsFramework.IconPickFrame,
			"bottomleft",
			12,
		...
		detailsFramework.IconPickFrame.customIconEntry =
			detailsFramework:CreateTextEntry(
				detailsFramework.IconPickFrame,
				function() end,
				200,
				20,
		...
		detailsFramework.IconPickFrame.customIconEntry:SetPoint(
			"left",
			detailsFramework.IconPickFrame.customIcon,
			"right",
			2,
		...
		detailsFramework.IconPickFrame.customIconEntry:SetHook("OnTextChanged", function()
			detailsFramework.IconPickFrame.preview:SetPoint(
				"bottom",
				detailsFramework.IconPickFrame.customIconEntry.widget,
				"top",
				0,
				2
			)
			detailsFramework.IconPickFrame.preview.icon:SetTexture(
				detailsFramework.IconPickFrame.customIconEntry:GetText()
			)
		...
		--close button
		local close_button =
			CreateFrame("button", nil, detailsFramework.IconPickFrame, "UIPanelCloseButton", "BackdropTemplate")
		...
		local accept_custom_icon = function()
			...
			detailsFramework:QuickDispatch(
				detailsFramework.IconPickFrame.callback,
				path,
				detailsFramework.IconPickFrame.param1,
				detailsFramework.IconPickFrame.param2
			)
		...
		detailsFramework.IconPickFrame.customIconAccept =
			detailsFramework:CreateButton(
				detailsFramework.IconPickFrame,
				accept_custom_icon,
				82,
				20,
		...
		detailsFramework.IconPickFrame.customIconAccept:SetPoint(
			"left",
			detailsFramework.IconPickFrame.customIconEntry,
			"right",
			2,
			...
			for i = 1, GetNumSpellTabs() do
				...
				for j = offset, tabEnd - 1 do
					...
					if (spellType ~= "FLYOUT") then
						MACRO_ICON_FILENAMES[index] =
							si and si.iconID or GetSpellBookItemTexture(j, SPELLBOOK_BANK_PLAYER) or 0
		...
		local scroll =
			detailsFramework:CreateScrollBox(
				detailsFramework.IconPickFrame,
				"DetailsFrameworkIconPickFrameScroll",
				updateIconScroll,
				{},
				width,
				height,
		...
		function scroll.RefreshIcons()
			...
			if (filter and filter ~= "") then
				...
				for i = 1, #MACRO_ICON_FILENAMES do
					if (MACRO_ICON_FILENAMES[i] and type(
						MACRO_ICON_FILENAMES[i]
					) == "string" and MACRO_ICON_FILENAMES[i]:lower():find(filter)) then
						if not index then
							index = 1
							local t = {}
							iconList[#iconList + 1] = t
...
function detailsFramework:ShowPanicWarning(text)
	if not detailsFramework.PanicWarningWindow then
		detailsFramework.PanicWarningWindow =
			CreateFrame("frame", "DetailsFrameworkPanicWarningWindow", UIParent, "BackdropTemplate")
		detailsFramework.PanicWarningWindow:SetHeight(80)
		detailsFramework.PanicWarningWindow:SetBackdrop({
			bgFile = [[Interface\Tooltips\UI-Tooltip-Background]],
			tileSize = 64,
			tile = true,
		})
		...
		detailsFramework.PanicWarningWindow.text =
			detailsFramework.PanicWarningWindow:CreateFontString(nil, "overlay", "GameFontNormal")
...
local SimplePanel_frame_backdrop = {
	edgeFile = [[Interface\Buttons\WHITE8X8]],
	edgeSize = 1,
	bgFile = [[Interface\Tooltips\UI-Tooltip-Background]],
	tileSize = 64,
...
---@field thumb texture
function detailsFramework:CreateScaleBar(
frame,
	config,
	bNoRightClick --~scale
)
	...
	local scaleBar, text =
		detailsFramework:CreateSlider(
			...
			nil,
			"Scale:",
			detailsFramework:GetTemplate("slider", "OPTIONS_SLIDER_TEMPLATE"),
			detailsFramework:GetTemplate("font", "ORANGE_FONT_TEMPLATE")
		)
	...
	editbox:SetBackdrop({
		bgFile = [[Interface\ACHIEVEMENTFRAME\UI-GuildAchievement-Parchment-Horizontal-Desaturated]],
		edgeFile = [[Interface\Buttons\WHITE8X8]],
		tile = true,
		edgeSize = 1,
		tileSize = 64,
	...
	editbox:SetScript("OnEditFocusGained", function() end)
	...
	editbox:SetScript("OnEscapePressed", function()
		if bNoRightClick then return end
		editbox:ClearFocus()
		editbox:Hide()
...
function detailsFramework:CreateSimplePanel(parent, width, height, title, frameName, panelOptions, savedVariableTable)
	...
	if (savedVariableTable and frameName and not savedVariableTable[frameName]) then
		savedVariableTable[frameName] = {scale = 1}
	end
...
local Panel1PxSetLocked = function(self, lock_state)
	if (type(lock_state) ~= "boolean") then return end
	if lock_state then
		-- lock it
		self.IsLocked = false
...
function detailsFramework:RestoreFramePosition(frame)
	if (frame.db and frame.db.position) then
		...
		frame:SetPoint(
			...
			UIParent,
			"center",
			frame.db.position.x * UIscale / scale,
			frame.db.position.y * UIscale / scale
		)
	end
end
...
function detailsFramework:ShowPromptPanel(message, trueCallback, falseCallback, dontOverride, width, promptName)
	if not DetailsFrameworkPromptSimple then
		...
		promptFrame.ShowAnimation = detailsFramework:CreateAnimationHub(
			promptFrame,
			function()
		...
		promptFrame.FlashAnimation = detailsFramework:CreateAnimationHub(
			promptFrame.FlashTexture,
			function()
				promptFrame.FlashTexture:Show()
			end,
			function()
				promptFrame.FlashTexture:Hide()
			end
		)
	...
	assert(
		type(trueCallback) == "function" and type(falseCallback) == "function",
		"ShowPromptPanel expects two functions."
	)

	if dontOverride then
		if detailsFramework.promtp_panel:IsShown() then return end
	end
...
function detailsFramework:ShowTextPromptPanel(message, callback)
	if not detailsFramework.text_prompt_panel then
		...
		promptFrame:SetScript("OnDragStart", function()
			promptFrame:StartMoving()
		end)
		promptFrame:SetScript("OnDragStop", function()
			promptFrame:StopMovingOrSizing()
		end)
		promptFrame:SetScript("OnMouseDown", function(self, button)
			if (button == "RightButton") then
				promptFrame.EntryBox:ClearFocus()
				promptFrame:Hide()
			end
		end)
		...
		local buttonFalse = detailsFramework:CreateButton(
			promptFrame,
			function()
				promptFrame.textbox:ClearFocus()
				promptFrame:Hide()
			end,
			60,
			20,
...
local create_box = function(self, next_box)
	...
	local border =
		detailsFramework:NewImage(self.Graphic, [[Interface\DialogFrame\UI-DialogBox-Gold-Corner]], 30, 30, "artwork")
	border:SetPoint("center", box, "center", -3, -4)
	border:SetTexture([[Interface\DialogFrame\UI-DialogBox-Gold-Corner]])

	local checktexture =
		detailsFramework:NewImage(self.Graphic, [[Interface\Buttons\UI-CheckBox-Check]], 18, 18, "overlay")
...
local chart_panel_add_overlay = function(self, overlayData, color, name, icon)
	...
	if (type(overlayData) == "number") then
		...
		draw_overlay(
			self,
			self.Overlays[self.OverlaysAmount],
			self.OData[overlay_index][1],
			self.OData[overlay_index][2]
		)
	else
...
local chart_panel_add_data = function(self, graphicData, color, name, elapsedTime, lineTexture, smoothLevel, firstIndex)
	...
	if not smoothLevel then
		while (index <= #content - 2) do
			local value =
				(content[index - 2] + content[index - 1] + content[index] + content[index + 1] + content[index + 2]) / 5 --normalize
	...
	if firstIndex then
		table.insert(LibGraphChartFrame.Data, 1, {
			Points = builtData,
			Color = color or line_default_color,
			lineTexture = newLineTexture,
...
function detailsFramework:CreateChartPanel(parent, width, height, name)
	...
	local title =
		detailsFramework:NewLabel(chartFrame, nil, "$parentTitle", "chart_title", "Chart!", nil, 20, {1, 1, 0})
	...
	--graphic
	local g =
		LibStub:GetLibrary("LibGraph-2.0"):CreateGraphLine(
			name .. "Graphic",
			chartFrame,
			"topleft",
			"topleft",
	...
	local bottom_texture =
		detailsFramework:NewImage(chartFrame, nil, 702, 25, "background", nil, nil, "$parentBottomTexture")
...
---@return df_scrollbox
function detailsFramework:CreateScrollBox(
parent,
	name,
	refreshFunc,
	data,
...
function detailsFramework:CreateResizeGrips(parent, options, leftResizerName, rightResizerName)
	...
	local leftResizer =
		_G.CreateFrame("button", leftResizerName or (parentName and "$parentLeftResizer"), parent, "BackdropTemplate")
	local rightResizer =
		_G.CreateFrame("button", rightResizerName or (parentName and "$parentRightResizer"), parent, "BackdropTemplate")
...
detailsFramework.TitleFunctions = {SetTitle = function(self, titleText, titleColor, font, size)
...
function detailsFramework:CreateTitleBar(parent, titleText)
	local titleBar =
		CreateFrame("frame", parent:GetName() and parent:GetName() .. "TitleBar" or nil, parent, "BackdropTemplate")
	...
	local closeButton =
		CreateFrame(
			"button",
			titleBar:GetName() and titleBar:GetName() .. "CloseButton" or nil,
			titleBar,
			"BackdropTemplate"
		)
	closeButton:SetSize(16, 16)
	...
	local titleLabel =
		titleBar:CreateFontString(
			titleBar:GetName() and titleBar:GetName() .. "TitleText" or nil,
			"overlay",
			"GameFontNormal"
		)
...
detailsFramework.RadioGroupCoreFunctions = {
	...
	CreateCheckbox = function(self)
		...
		if self.options.checked_texture then
			checkbox:SetCheckedTexture(
				self.options.checked_texture,
				self.options.checked_texture_offset_x,
				self.options.checked_texture_offset_y
			)
		end
	...
	RadioOnClick = function(checkbox, fixedParam, value)
		...
		if radioGroup.options.on_click_option then
			detailsFramework:QuickDispatch(
				radioGroup.options.on_click_option,
				radioGroup,
				checkbox,
				checkbox._param,
				checkbox._optionid
	...
	RefreshCheckbox = function(self, checkbox, optionTable, optionId)
		...
		local bIsChecked =
			(type(optionTable.selected) == "boolean" and optionTable.selected) or (type(
				optionTable.get
			) == "function" and detailsFramework:Dispatch(optionTable.get)) or false
		...
		checkbox.__width =
			width + (checkbox.Icon:IsShown() and (checkbox.Icon:GetWidth() + 2)) + checkbox.Label:GetStringWidth() + 2
	...
	Select = function(self, option)
		...
		if thisCheckbox then
			...
			if callbackFunc then
				detailsFramework.RadioGroupCoreFunctions.RadioOnClick(
					thisCheckbox,
					thisCheckbox:GetFixedParameter(),
					true
				)
			end
		end
...
function detailsFramework:CreateCheckboxGroup(parent, radioOptions, name, options, anchorOptions)
	...
	if newCheckboxGroup.options.title then
		local titleLabel =
			detailsFramework:CreateLabel(
				newCheckboxGroup,
				newCheckboxGroup.options.title,
				detailsFramework:GetTemplate("font", "ORANGE_FONT_TEMPLATE")
			)
...
function detailsFramework:CreateRadionGroup(
parent,
	radioOptions,
	name,
	options,
	anchorOptions --alias for miss spelled old function
)
...
detailsFramework.DataScrollFunctions = {
	RefreshScroll = function(self, data, offset, totalLines)
		...
		if self.SortAlphabetical then
			table.sort(currentData, function(t1, t2)
				return t1[1] < t2[1]
			end)
		end
	...
	CreateLine = function(self, index)
		...
		local title =
			detailsFramework:CreateLabel(line, "", detailsFramework:GetTemplate("font", self.options.title_template))
		local date =
			detailsFramework:CreateLabel(line, "", detailsFramework:GetTemplate("font", self.options.title_template))
		local text =
			detailsFramework:CreateLabel(line, "", detailsFramework:GetTemplate("font", self.options.text_tempate))
	...
	UpdateLine = function(line, lineIndex, data)
		...
		if line:GetParent().OnUpdateLineHook then
			detailsFramework:CoreDispatch(
				(line:GetName() or "ScrollBoxDataScrollUpdateLineHook") .. ":UpdateLineHook()",
				line:GetParent().OnUpdateLineHook,
				line,
				lineIndex,
				data
			)
...
function detailsFramework:CreateDataScrollFrame(parent, name, options)
	...
	local newScroll =
		detailsFramework:CreateScrollBox(
			parent,
			name,
			optionsTable.refresh_func,
			optionsTable.data,
			optionsTable.width,
			optionsTable.height,
			optionsTable.line_amount,
			optionsTable.line_height
		)
...
function detailsFramework:CreateNewsFrame(parent, name, options, newsTable, db)
	local f =
		detailsFramework:CreateSimplePanel(
			parent,
			400,
			700,
			options and options.title or default_newsframe_options.title,
			name,
			{UseScaleBar = db and true},
			db
		)
...
function detailsFramework:BuildStatusbarAuthorInfo(f, addonBy, authorsNameString)
	local authorName =
		detailsFramework:CreateLabel(
			f,
			"" .. (addonBy or "An addon by ") .. "|cFFFFFFFF" .. (authorsNameString or "Terciob") .. "|r"
		)
	...
	local options_dropdown_template = detailsFramework:GetTemplate("dropdown", "OPTIONS_DROPDOWN_TEMPLATE")
	local discordTextEntry =
		detailsFramework:CreateTextEntry(f, function() end, 200, 18, "DiscordTextBox", _, _, options_dropdown_template)
...
local statusbar_default_options = {attach = "bottom"} --bottomleft from statusbar attach to bottomleft of the frame | other option is "top": topleft attach to bottomleft
...
local timeline_options = {
	...
	block_on_enter = function(self) end,
	block_on_leave = function(self) end,
}
...
detailsFramework.TimeLineFunctions = {
		...
		if not line then
			...
			if self.options.show_elapsed_timeline then
				line:SetPoint(
					...
					"topleft",
					1,
					-((index - 1) * (self.options.line_height + 1)) - 2 - self.options.elapsed_timeline_height
				)
			else
			...
			local text =
				detailsFramework:CreateLabel(
					line,
					"",
					detailsFramework:GetTemplate("font", self.options.title_template)
				)
	...
	AdjustScale = function(self, index) end,
	--todo
	...
	RefreshTimeLine = function(self)
		...
		self.horizontalSlider:SetValue(
			detailsFramework:MapRangeClamped(oldMin, oldMax, 0, newMaxValue, self.horizontalSlider:GetValue())
		)
...
function detailsFramework:CreateTimeLineFrame(parent, name, options, timelineOptions)
	...
	frameCanvas.elapsedTimeFrame =
		detailsFramework:CreateElapsedTimeFrame(
			frameBody,
			frameCanvas:GetName() and frameCanvas:GetName() .. "ElapsedTimeFrame",
			timelineOptions
		)
	...
	local horizontalSlider =
		CreateFrame("slider", frameCanvas:GetName() .. "HorizontalSlider", parent, "BackdropTemplate")
	...
	scaleSlider:SetValue(
		detailsFramework:GetRangeValue(frameCanvas.options.scale_min, frameCanvas.options.scale_max, 0.5)
	)
	...
	frameBody:SetScript("OnMouseDown", function(self, button)
		...
		frameBody:SetScript("OnUpdate", function(self, deltaTime)
			...
			horizontalSlider:SetValue(
				current + (deltaX * 1.2) * ((IsShiftKeyDown() and 2) or (IsAltKeyDown() and 0.5) or 1)
			)
			self.MouseX = x
		end)
	end)
...
function detailsFramework:ShowErrorMessage(errorMessage, titleText)
	if not detailsFramework.ErrorMessagePanel then
		...
		f.ShowAnimation = detailsFramework:CreateAnimationHub(
			f,
			function()
		...
		f.FlashAnimation = detailsFramework:CreateAnimationHub(
			f.FlashTexture,
			function()
				f.FlashTexture:Show()
			end,
		...
		for i = 1, (listBox.headerLength + 2) do --+2 to add the delete button and index
			...
			if headerColumn.isDelete then
				local deleteButton =
					detailsFramework:CreateButton(
						line,
						detailsFramework.ListboxFunctions.deleteEntry,
						20,
						self.lineHeight,
						"X",
						listBox.data,
...
function detailsFramework:CreateListBox(parent, name, data, options, headerTable, headerOptions)
	...
	headerOptions = headerOptions or {padding = 2}
	...
	table.insert(headerTable, 1, {
		text = "#",
		width = 20,
		isIndex = true,
	}) --isDelete signals the createScrollLine() to make the delete button for the line
	...
	local scrollBox =
		detailsFramework:CreateScrollBox(
			frameCanvas,
			"$parentScrollbox",
			frameCanvas.scrollRefresh,
			data,
			width - 4,
			height - header:GetHeight() - 12 - 24,
			lineAmount,
			lineHeight
		)
	...
	local addLineButton =
		detailsFramework:CreateButton(
			frameCanvas,
			detailsFramework.ListboxFunctions.addEntry,
			80,
			20,
			"Add",
			nil,
