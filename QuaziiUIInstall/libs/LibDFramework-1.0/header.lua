...
detailsFramework.HeaderFunctions = {
	...
	AlignWithHeader = function(self, headerFrame, anchor)
		...
		for i = 1, #self.FramesToAlign do
			...
			if columnHeader then
				...
				uiObject:SetPoint(
					columnHeader.columnAlign,
					self,
					anchor,
					columnHeader.XPosition + columnHeader.columnOffset + offset,
					0
				)
			end
		end
	...
	OnClick = function(columnHeader, buttonClicked)
		...
		if not headerFrame.columnSelected then return end

		--check if this column has 'canSort' key, otherwise ignore the click
		if not columnHeader.columnData.canSort then return end
		...
		if headerFrame.options.header_click_callback then
			...
			local okay, errortext =
				pcall(
					headerFrame.options.header_click_callback,
					headerFrame,
					columnHeader,
	...
	OnMouseDown = function(self, buttonClicked)
		if (buttonClicked == "LeftButton") then  end
	end,
	---comment
	---@param self button
	---@param buttonClicked string
		...
		if not columnHeader then
			...
			resizerButton:SetScript("OnMouseDown", function()
				...
				if not columnHeader.bIsRezising then
					...
					columnHeader:SetResizeBounds(
						math.max(textLength, self.options.reziser_min_width),
						columnHeader:GetHeight(),
						self.options.reziser_max_width,
						columnHeader:GetHeight()
					)
			...
			resizerButton:SetScript("OnMouseUp", function()
				if columnHeader.bIsRezising then
					...
					if self.OnColumnSettingChangeCallback then --need to get the header name
						...
						xpcall(
							self.OnColumnSettingChangeCallback,
							geterrorhandler(),
							self,
							"width",
							columnName,
