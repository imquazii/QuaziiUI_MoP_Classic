	...
	if not self.is_checkbox then
		if not self.lock_texture then
			DF:NewImage(
				...
				12,
				"overlay",
				{0.0546875, 0.9453125, 0.0703125, 0.9453125},
				"lock_texture",
				"$parentLockTexture"
			)
...
local OnEnter = function(slider)
	...
	if rawget(object, "lockdown") then return end
...
local sliderButtonsParentFrame =
	DetailsFrameworkSliderButtons1 or CreateFrame(
		"frame",
		"DetailsFrameworkSliderButtons1",
		UIParent,
		"BackdropTemplate"
	)
...
local buttonPlus =
	DetailsFrameworkSliderButtonsPlusButton or CreateFrame(
		"button",
		"DetailsFrameworkSliderButtonsPlusButton",
		sliderButtonsParentFrame,
		"BackdropTemplate"
	)
local buttonMinor =
	DetailsFrameworkSliderButtonsMinorButton or CreateFrame(
		"button",
		"DetailsFrameworkSliderButtonsMinorButton",
		sliderButtonsParentFrame,
		"BackdropTemplate"
	)
	...
	if sliderObject.fine_tuning then
		...
		if (editboxShowingValue and DFSliderMetaFunctions.editbox_typevalue:IsShown()) then
			DFSliderMetaFunctions.editbox_typevalue:SetText(
				tostring(string.format("%.2f", currentValueOnSlider + sliderObject.fine_tuning))
			)
		end
	else
...
function DFSliderMetaFunctions:SetTemplate(template)
	...
	if template.is_checkbox then
		...
		self:SetCheckedTexture(
			template.checked_texture,
			template.checked_xoffset or 0,
			template.checked_yoffset or 0,
			template.checked_size_percent or 0.7,
			template.checked_color
		)
	end
...
parent,
	...
	SliderObject.slider:SetBackdrop({
		edgeFile = "Interface\\Buttons\\UI-SliderBar-Border",
		edgeSize = 8,
	})
...
DF.AdjustmentSliderFunctions = {
	...
	RunCallback = function(adjustmentSlider, valueX, valueY, isLiteral)
		local result, errorText =
			pcall(
				adjustmentSlider.callback,
				adjustmentSlider,
				valueX,
				valueY,
				isLiteral,
				adjustmentSlider:DumpPayload()
			)
		if not result then
			...
			elseif (xDelta ~= 0 or yDelta ~= 0) then
				if (adjustmentSlider.buttonPressed == "center") then
					...
					local mouseDirection =
						CreateVector2D(mouseX - adjustmentSlider.initialMouseX, mouseY - adjustmentSlider.initialMouseY)
					local length = DF:MapRangeClamped(-100, 100, -1, 1, mouseDirection:GetLength())
					mouseDirection:Normalize()
					mouseDirection:ScaleBy(speed * length)
					adjustmentSlider.centerArrowArtwork:SetPoint(
						"center",
						adjustmentSlider.centerButton.widget,
						"center",
						mouseDirection:GetXY()
					)
				end

				local scaleResultBy = adjustmentSlider:GetScaleFactor()
				DF.AdjustmentSliderFunctions.RunCallback(
					adjustmentSlider,
					horizontalValue * scaleResultBy,
					verticalValue * scaleResultBy,
					false
				)
...
local createAdjustmentSliderFrames = function(parent, options, name)
	...
	local leftButton =
		DF:CreateButton(adjustmentSlider, function() end, 20, 20, "", "left", -1, nil, nil, name .. "LeftButton")
	local rightButton =
		DF:CreateButton(adjustmentSlider, function() end, 20, 20, "", "right", 1, nil, nil, name .. "RightButton")
	...
	--center button
	local centerButton =
		DF:CreateButton(adjustmentSlider, function() end, 20, 20, "", "center", 0, nil, nil, name .. "CenterButton")
