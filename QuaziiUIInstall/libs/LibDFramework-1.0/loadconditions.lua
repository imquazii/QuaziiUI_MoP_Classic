...
function detailsFramework:CreateLoadFilterParser(callback)
	...
	filterFrame:SetScript("OnEvent", function(self, event, ...)
		...
		elseif (event == "PLAYER_ROLES_ASSIGNED") then
			...
			if (detailsFramework.CurrentPlayerRole == assignedRole) then return end
...
function detailsFramework:OpenLoadConditionsPanel(optionsTable, callback, frameOptions)
	...
	if not loadConditionsFrame then
		loadConditionsFrame =
			detailsFramework:CreateSimplePanel(UIParent, 1024, 600, "Load Conditions", "loadConditionsFrame")
		...
		for _, classTable in pairs(detailsFramework:GetClassList()) do
			table.insert(classes, {
				...
				get = function()
					return loadConditionsFrame.OptionsTable.class[classTable.FileString]
				end,
		...
		local classGroup = detailsFramework:CreateCheckboxGroup(
			loadConditionsFrame,
			classes,
			nil,
			{
		...
		classGroup:SetPoint(
			...
			loadConditionsFrame,
			"topleft",
			anchorPositions.class[1],
			anchorPositions.class[2]
		)
		...
		if IS_WOW_PROJECT_MAINLINE then
			...
			for _, specID in ipairs(detailsFramework:GetClassSpecIDs(select(2, UnitClass("player")))) do
				local specID, specName, specDescription, specIcon, specBackground, specRole, specClass =
					DetailsFramework.GetSpecializationInfoByID(specID)
				table.insert(specs, {
					name = specName,
					set = loadConditionsFrame.OnRadioCheckboxClick,
					param = specID,
					get = function()
						return loadConditionsFrame.OptionsTable.spec[specID] or loadConditionsFrame.OptionsTable.spec[specID .. ""]
					end,
					texture = specIcon,
				})
			end

			local specGroup = detailsFramework:CreateCheckboxGroup(
				loadConditionsFrame,
				specs,
				nil,
				{
			...
			specGroup:SetPoint(
				...
				loadConditionsFrame,
				"topleft",
				anchorPositions.spec[1],
				anchorPositions.spec[2]
			)
		...
		for _, raceTable in ipairs(detailsFramework:GetCharacterRaceList()) do
			table.insert(raceList, {
				...
				get = function()
					return loadConditionsFrame.OptionsTable.race[raceTable.FileString]
				end,
			})
		end

		local raceGroup = detailsFramework:CreateCheckboxGroup(loadConditionsFrame, raceList, nil, {
			width = 200,
			height = 200,
		...
		if IS_WOW_PROJECT_MAINLINE then
			...
			--10.0 talents schema
			local talentList = {}
			local talentGroup = detailsFramework:CreateCheckboxGroup(
				loadConditionsFrame,
				talentList,
				nil,
				{
			...
			talentGroup:SetPoint(
				"topleft",
				loadConditionsFrame,
				"topleft",
				anchorPositions.talent[1],
				anchorPositions.talent[2]
			)
			...
			do
				if false then --disabled, isn't in use
					...
					otherTalents.Texture =
						detailsFramework:CreateImage(
							otherTalents,
					...
					local removeTalent = function(_, _, talentID)
						...
						GameCooltip2:Hide()
						loadConditionsFrame.OnRadioStateChanged(
							talentGroup,
							loadConditionsFrame.OptionsTable[talentGroup.DBKey]
						)
					...
					local buildTalentMenu = function()
						...
						GameCooltip2:AddLine(
							"select a talent to remove it (added from a different spec or character)",
							"",
							1,
							"orange",
							"orange",
						...
						for talentID, _ in pairs(loadConditionsFrame.OptionsTable.talent) do
							if (type(talentID) == "number" and not detailsFramework.table.find(
								indexedTalents,
								talentID
							)) then
		...
		if IS_WOW_PROJECT_MAINLINE then
			...
			for _, talentTable in ipairs(detailsFramework:GetCharacterPvPTalents()) do
				table.insert(pvpTalentList, {
					...
					param = talentTable.ID,
					get = function()
						return loadConditionsFrame.OptionsTable.pvptalent[talentTable.ID] or loadConditionsFrame.OptionsTable.pvptalent[talentTable.ID .. ""]
					end,
					texture = talentTable.Texture,
				})
			end
			local pvpTalentGroup = detailsFramework:CreateCheckboxGroup(
				loadConditionsFrame,
				pvpTalentList,
				nil,
				{
			...
			pvpTalentGroup:SetPoint(
				"topleft",
				loadConditionsFrame,
				"topleft",
				anchorPositions.pvptalent[1],
				anchorPositions.pvptalent[2]
			)
			...
			do
				if false then
					...
					local removeTalent = function(_, _, talentID)
						...
						loadConditionsFrame.OnRadioStateChanged(
							pvpTalentGroup,
							loadConditionsFrame.OptionsTable[pvpTalentGroup.DBKey]
						)
		...
		for _, groupTable in ipairs(detailsFramework:GetGroupTypes()) do
			table.insert(groupTypes, {
				...
				param = groupTable.ID,
				get = function()
					return loadConditionsFrame.OptionsTable.group[groupTable.ID] or loadConditionsFrame.OptionsTable.group[groupTable.ID .. ""]
				end,
			})
		end
		local groupTypesGroup = detailsFramework:CreateCheckboxGroup(
			loadConditionsFrame,
			groupTypes,
			nil,
			{
		...
		groupTypesGroup:SetPoint(
			"topleft",
			loadConditionsFrame,
			"topleft",
			anchorPositions.group[1],
			anchorPositions.group[2]
		)
		...
		for _, roleTable in ipairs(detailsFramework:GetRoleTypes()) do
			table.insert(roleTypes, {
				...
				param = roleTable.ID,
				get = function()
					return loadConditionsFrame.OptionsTable.role[roleTable.ID] or loadConditionsFrame.OptionsTable.role[roleTable.ID .. ""]
				end,
			})
		end

		local roleTypesGroup = detailsFramework:CreateCheckboxGroup(loadConditionsFrame, roleTypes, nil, {
			width = 200,
			height = 200,
			title = "Role Types",
			backdrop_color = {0, 0, 0, 0},
		})
		roleTypesGroup:SetPoint(
			"topleft",
			loadConditionsFrame,
			"topleft",
			anchorPositions.role[1],
			anchorPositions.role[2]
		)
		...
		if IS_WOW_PROJECT_MAINLINE then
			...
			for i = 2, 1000 do
				...
				if (affixName and not deprecatedAffixes[i]) then
					table.insert(affixes, {
						...
						param = i,
						get = function()
							return loadConditionsFrame.OptionsTable.affix[i] or loadConditionsFrame.OptionsTable.affix[i .. ""]
						end,
						texture = texture,
					})
				end
			...
			local affixTypesGroup = detailsFramework:CreateCheckboxGroup(
				loadConditionsFrame,
				affixes,
				nil,
				{
			...
			affixTypesGroup:SetPoint(
				"topleft",
				loadConditionsFrame,
				"topleft",
				anchorPositions.affix[1],
				anchorPositions.affix[2]
			)
		...
		encounterIDLabel:SetPoint(
			"topleft",
			loadConditionsFrame,
			"topleft",
			anchorPositions.encounter_ids[1],
			anchorPositions.encounter_ids[2]
		)
		...
		encounterIDEditbox.tooltip =
			"Enter multiple IDs separating with a whitespace.\nExample: 35 45 95\n\nSanctum of Domination:\n"

		for _, encounterTable in ipairs(detailsFramework:GetCLEncounterIDs()) do
			encounterIDEditbox.tooltip =
				encounterIDEditbox.tooltip .. encounterTable.ID .. " - " .. encounterTable.Name .. "\n"
		end
		...
		local mapIDEditbox = detailsFramework:CreateTextEntry(
			loadConditionsFrame,
			function()
				loadConditionsFrame.RunCallback()
			end,
		...
		mapIDLabel:SetPoint(
			...
			loadConditionsFrame,
			"topleft",
			anchorPositions.map_ids[1],
			anchorPositions.map_ids[2]
		)
				...
				for _, talentTable in ipairs(allTalents) do
					if talentTable.ID then
						table.insert(talentList, {
							...
							param = talentTable.ID,
							get = function()
								return loadConditionsFrame.OptionsTable.talent[talentTable.ID] or loadConditionsFrame.OptionsTable.talent[talentTable.ID .. ""]
							end,
							texture = talentTable.Texture,
