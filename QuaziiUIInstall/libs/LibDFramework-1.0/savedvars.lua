...
function detailsFramework.SavedVars.SetProfile(addonObject, profileName, bCopyFromCurrentProfile)
	...
	---@type profile
	local profileTable =
		detailsFramework.SavedVars.GetProfile(
			addonObject,
			bCreateIfNotFound,
			bCopyFromCurrentProfile and currentProfile or nil
		)
	...
	RefreshSelectProfileDropdown = function(self)
		...
		for profileId in pairs(profiles) do
			table.insert(dropdownOptions, {
				...
				label = profileId,
				onclick = callback,
				icon = [[Interface\CHATFRAME\UI-ChatIcon-BlizzardArcadeCollection]],
				iconsize = {16, 16},
			})
		end
...
function detailsFramework.SavedVars.CreateProfilePanel(addonObject, frameName, parentFrame, options)
	...
	local textentryTemplate, labelTemplate =
	...
	---@type df_dropdown
	local profileSelectionDropdown =
		detailsFramework:CreateDropDown(
			frame,
			onSelectProfileCallback,
			defaultValue,
			180,
			32,
			"ProfileSelectionDropdown",
			"$parentProfileSelectionDropdown",
			dropdownTemplate
		)
	...
	local onPressEnterCallback = function() end

	---@type df_textentry
	local profileNameTextEntry =
	...
	---@type df_button
	local createProfileButton = detailsFramework:CreateButton(
		frame,
		function()
			frame.OnClickCreateNewProfile(frame)
		end,
		100,
		32,
		"Create",
