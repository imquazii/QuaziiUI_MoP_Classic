...
function DropDownMetaFunctions:GetFrameForOption(
optionsTable,
	value --not tested
)
	if self.MyObject then
...
function DropDownMetaFunctions:NoOptionSelected()
	if self.no_options then return end
...
local runCallbackFunctionForButton = function(button)
	...
	if button.table.onclick then
		...
		local success, errorText =
			pcall(
				button.table.onclick,
				button:GetParent():GetParent():GetParent().MyObject,
				button.object.FixedValue,
				button.table.value
			)
		if not success then
			error(
				"Details! Framework: dropdown " .. button:GetParent():GetParent():GetParent().MyObject:GetName() .. " error: " .. errorText
			)
		end
		button:GetParent():GetParent():GetParent().MyObject:RunHooksForWidget(
			"OnOptionSelected",
			button:GetParent():GetParent():GetParent().MyObject,
			button.object.FixedValue,
			button.table.value
...
function DropDownMetaFunctions:Select(optionName, byOptionNumber, bOnlyShown, runCallback)
	...
	if (type(optionsTable) ~= "table") then
		error(
			"optionsTable for Dropdown:Select() is not of type 'table'. Check if the dropdown menu function is returning a table."
		)
	end
	...
	if (byOptionNumber and type(optionName) == "number") then
		...
		for i = 1, #optionsTable do
			local thisOption = optionsTable[i]
			if ((thisOption.label == optionName or thisOption.value == optionName) and isOptionVisible(
				self,
				thisOption
			)) then
	...
	if (not object.opened and not rawget(object, "lockdown")) then
		...
		if (optionsTable and optionsTable[1]) then
			...
			if object.OnMouseDownHook then
				local interrupt =
					object.OnMouseDownHook(button, buttontype, optionsTable, scrollFrame, scrollChild, selectedTexture)
...
function DF:BuildDropDownFontList(onClick, icon, iconTexcoord, iconSize, bIncludeDefault)
	...
	for name, fontPath in pairs(SharedMedia:HashTable("font")) do
		fontTable[#fontTable + 1] = {
			...
			font = fontPath,
			descfont = "abcdefg ABCDEFG",
		}
	end

	table.sort(fontTable, function(t1, t2)
		return t1.label < t2.label
	end)

	if bIncludeDefault then
		table.insert(fontTable, 1, {
			value = "DEFAULT",
			label = "DEFAULT",
			onclick = onClick,
			icon = icon,
...
function DF:CreateFontListGenerator(callback, bIncludeDefault)
	return function()
		return DF:BuildDropDownFontList(
			callback,
			[[Interface\AnimCreate\AnimCreateIcons]],
			{0, 32 / 128, 64 / 128, 96 / 128},
			16,
			bIncludeDefault
		)
	end
...
function DF:CreateAudioListGenerator(callback)
	local newGenerator = function()
		...
		table.sort(DF.AudioCues, function(t1, t2)
			return t1[1] < t2[1]
		end)
...
function DF:NewDropDown(parent, container, name, member, width, height, func, default, template)
	...
	dropDownObject.scroll =
		DF:NewScrollBar(scroll, _G[dropDownObject.dropdown:GetName() .. "_ScrollFrame" .. "_ScrollChild"], -18, -18)
