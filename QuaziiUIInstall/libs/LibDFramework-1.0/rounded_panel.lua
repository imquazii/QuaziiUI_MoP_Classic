...
local setCornerPoints = function(self, textures, width, height, xOffset, yOffset, bIsBorder)
	for cornerName, thisTexture in pairs(textures) do
		...
		if (not thisTexture.MaskTexture and bIsBorder) then
			...
			thisTexture.MaskTexture:SetTexture(
				[[Interface\Azerite\AzeriteGoldRingRank2]],
				"CLAMP",
				"CLAMP",
				"TRILINEAR"
			) --1940690
...
detailsFramework.RoundedCornerPanelMixin = {
	...
	CreateTitleBar = function(self, optionsTable)
		...
		local titleBar = detailsFramework:CreateRoundedPanel(self, "$parentTitleBar", {
			width = self.options.width - 6,
			height = self.options.titlebar_height,
		})
	...
	OnSizeChanged = function(self)
		...
		if (false and frameHeight < 32) then
			...
			if self.bHasBorder then
				...
				PixelUtil.SetSize(
					self.BorderEdgeTextures["Top"],
					horizontalEdgesNewSize + (self.options.horizontal_border_size_offset or 0),
					1
				)
				PixelUtil.SetSize(
					self.BorderEdgeTextures["Bottom"],
					horizontalEdgesNewSize + (self.options.horizontal_border_size_offset or 0),
					1
				)
			end
			...
			if self.bHasBorder then
				for _, thisTexture in pairs(self.BorderCornerTextures) do
					PixelUtil.SetSize(
						thisTexture,
						cornerWidth - self.cornerRoundness,
						cornerHeight - self.cornerRoundness
					)
...
function detailsFramework:AddRoundedCornersToFrame(frame, preset)
	...
	assert(
		frame and frame.GetObjectType and frame.SetPoint,
		"AddRoundedCornersToFrame(frame): frame must be a frame object."
	)

	if frame.__rcorners then return end
	...
	if preset.tab_side then
		if (preset.tab_side == "top") then
			...
			if frame.BorderCornerTextures["TopLeft"] then
				...
				frame.BorderCornerTextures["TopLeft"]:SetPoint(
					point1,
					relativeTo,
					point2,
					x,
					math.abs(preset.roundness - 16)
				)

				point1, relativeTo, point2, x, y = frame.BorderCornerTextures["TopRight"]:GetPoint(1)
				frame.BorderCornerTextures["TopRight"]:SetPoint(
					point1,
					relativeTo,
					point2,
					x,
		...
		elseif (preset.tab_side == "bottom") then
			...
			if frame.BorderCornerTextures["BottomLeft"] then
				...
				frame.BorderCornerTextures["BottomLeft"]:SetPoint(
					point1,
					relativeTo,
					point2,
					x,
				...
				frame.BorderCornerTextures["BottomRight"]:SetPoint(
					point1,
					relativeTo,
					point2,
					x,
