...
local setDropdownProperties = function(
parent,
	widget,
	widgetTable,
	currentXOffset,
	...
	if bAlignAsPairs then --regular
		...
		if not widget.highlightFrame then
			local highlightFrame =
				createOptionHighlightFrame(widget, label, (widgetWidth or 140) + nAlignAsPairsLength + 5)
...
local setToggleProperties = function(
parent,
	widget,
	widgetTable,
	currentXOffset,
...
local setRangeProperties = function(
parent,
	widget,
	widgetTable,
	currentXOffset,
...
local setColorProperties = function(
parent,
	widget,
	widgetTable,
	currentXOffset,
...
local setExecuteProperties = function(
parent,
	widget,
	widgetTable,
	currentXOffset,
	...
	if widgetTable.icontexture then
		widget:SetIcon(
			widgetTable.icontexture,
			widget:GetHeight() - 2,
			widget:GetHeight() - 2,
			nil,
			widgetTable.icontexcoords,
			nil,
			nil,
			2
		)
...
local setTextEntryProperties = function(
parent,
	widget,
	widgetTable,
	currentXOffset,
...
local getMenuWidgetVolative = function(parent, widgetType, indexTable)
		...
		if not widgetObject then
			widgetObject =
				detailsFramework:CreateLabel(
					...
					"",
					nil,
					"$parentWidget" .. widgetType .. indexTable[widgetType],
					"overlay"
				)
	...
	elseif (widgetType == "dropdown") then
		widgetObject = parent.widget_list_by_type[widgetType][indexTable[widgetType]]
		if not widgetObject then
			widgetObject = detailsFramework:CreateDropDown(
				parent,
				function()
					return {}
				end,
			...
			widgetObject.hasLabel =
				detailsFramework:CreateLabel(
					...
					"",
					nil,
					"$parentWidget" .. widgetType .. indexTable[widgetType] .. "label",
					"overlay"
				)
			table.insert(parent.widget_list, widgetObject)
			table.insert(parent.widget_list_by_type[widgetType], widgetObject)
	...
	elseif (widgetType == "color") then
		...
		if not widgetObject then
			widgetObject =
				detailsFramework:CreateColorPickButton(
					parent,
					"$parentWidget" .. widgetType .. indexTable[widgetType],
					nil,
					function() end,
...
local getNamePhraseID = function(widgetTable, languageAddonId, languageTable, bIgnoreEmbed)
	...
	local hasValue =
		detailsFramework.Language.DoesPhraseIDExistsInDefaultLanguage(languageAddonId, embedPhraseId or keyName)
...
local getNamePhraseText = function(languageTable, widgetTable, useColon, languageAddonId)
	...
	local namePhrase =
		languageTable and (languageTable[namePhraseId] or languageTable[widgetTable.namePhraseId] or languageTable[widgetTable.name])

	if (bWasEmbed and widgetTable.name) then
		namePhrase = widgetTable.name:gsub("@" .. namePhraseId .. "@", namePhrase)
	end

	return namePhrase or formatOptionNameWithColon(
		widgetTable.name,
		useColon
	) or widgetTable.namePhraseId or widgetTable.name or "-?-"
end

--volatile menu can be called several times, each time all settings are reset and a new menu is built reusing the widgets
function detailsFramework:BuildMenuVolatile(
parent,
	menuOptions,
	xOffset,
	yOffset,
	...
	local bUseBoxFirstOnAllWidgets,
		widgetWidth,
		widgetHeight,
		bAlignAsPairs,
		nAlignAsPairsLength,
		nAlignAsPairsSpacing,
		bUseScrollFrame,
		languageAddonId,
		bAttachSliderButtonsToLeft
		...
		if not widgetTable.hidden then
			...
			if not widgetTable.novolatile then
				...
				elseif (widgetTable.type == "label" or widgetTable.type == "text") then
					...
					local namePhrase =
						(languageTable and (languageTable[namePhraseId] or languageTable[widgetTable.namePhraseId] or languageTable[widgetTable.name])) or (widgetTable.get and widgetTable.get()) or widgetTable.text or widgetTable.namePhraseId or ""
				...
				elseif widgetTable.type:find("select") then
					assert(
						widgetTable.get,
						"DetailsFramework:BuildMenu: .get() not found in the widget table for 'select'"
					)
					...
					do
						if (widgetTable.type == "selectfont") then
							local func =
								detailsFramework:CreateFontListGenerator(widgetTable.set, widgetTable.include_default)
				...
				if bUseScrollFrame then
					if (widgetTable.type == "breakline") then
						...
						if bAlignAsPairs then
							currentXOffset =
								currentXOffset + nAlignAsPairsLength + (widgetWidth or maxWidgetWidth) + nAlignAsPairsSpacing
						else
...
function detailsFramework:BuildMenu(
parent,
	menuOptions,
	xOffset,
	yOffset,
		...
		if not widgetTable.hidden then
			...
			elseif (widgetTable.type == "label" or widgetTable.type == "text") then
				...
				local label =
					detailsFramework:CreateLabel(
						parent,
						"",
						widgetTable.text_template or textTemplate or widgetTable.size,
						widgetTable.color,
						widgetTable.font,
						nil,
						"$parentWidget" .. index,
			...
			elseif widgetTable.type:find("select") then
				...
				do
					if (widgetTable.type == "selectfont") then
						dropdown =
							detailsFramework:CreateFontDropDown(
								parent,
								widgetTable.set,
								widgetTable.get(),
								widgetWidth or 140,
								widgetHeight or defaultHeight,
								nil,
								"$parentWidget" .. index,
								dropdownTemplate,
								widgetTable.include_default
							)
					elseif (widgetTable.type == "selectcolor") then
						dropdown =
							detailsFramework:CreateColorDropDown(
								parent,
								widgetTable.set,
								widgetTable.get(),
								widgetWidth or 140,
					...
					elseif (widgetTable.type == "selectanchor") then
						dropdown =
							detailsFramework:CreateAnchorPointDropDown(
								parent,
								widgetTable.set,
								widgetTable.get(),
								widgetWidth or 140,
					...
					elseif (widgetTable.type == "selectoutline") then
						dropdown =
							detailsFramework:CreateOutlineDropDown(
								parent,
								widgetTable.set,
								widgetTable.get(),
								widgetWidth or 140,
					...
					elseif (widgetTable.type == "selectaudio") then
						dropdown =
							detailsFramework:CreateAudioDropDown(
								parent,
								widgetTable.set,
								widgetTable.get(),
								widgetWidth or 140,
				...
				DetailsFramework.Language.RegisterTableKeyWithDefault(
					languageAddonId,
					dropdown,
					"have_tooltip",
					descPhraseId,
				...
				if languageAddonId then
					detailsFramework.Language.RegisterCallback(languageAddonId, function(addonId, languageId, ...)
						dropdown:Select(dropdown:GetValue())
					end)
			...
			elseif (widgetTable.type == "color") then
				...
				local colorpick =
					detailsFramework:NewColorPickButton(
						parent,
						"$parentWidget" .. index,
						nil,
						widgetTable.set,
			...
			elseif (widgetTable.type == "textentry") then
				...
				local textentry =
					detailsFramework:CreateTextEntry(
						parent,
						widgetTable.func or widgetTable.set,
						widgetWidth or 120,
						widgetHeight or 18,
						nil,
						"$parentWidget" .. index,
