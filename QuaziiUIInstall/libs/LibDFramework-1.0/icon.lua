...
detailsFramework.IconMixin = {
	...
	CreateIcon = function(self, iconName)
		...
		local cooldownFrame =
			CreateFrame("cooldown", "$parentCooldown", iconFrame, "CooldownFrameTemplate, BackdropTemplate")
	...
	GetIcon = function(self)
		...
		if not iconFrame then
			...
			iconFrame.CountdownText:SetPoint(
				self.options.text_anchor or "center",
				iconFrame,
				self.options.text_rel_anchor or "center",
				self.options.text_x_offset or 0,
				self.options.text_y_offset or 0
			)
			iconFrame.Desc:ClearAllPoints()
			iconFrame.Desc:SetPoint(
				self.options.desc_text_anchor or "bottom",
				iconFrame,
				self.options.desc_text_rel_anchor or "top",
				self.options.desc_text_x_offset or 0,
				self.options.desc_text_y_offset or 2
			)

			iconFrame.StackText:ClearAllPoints()
			iconFrame.StackText:SetPoint(
				self.options.stack_text_anchor or "center",
				iconFrame,
				self.options.stack_text_rel_anchor or "center",
				self.options.stack_text_x_offset or 0,
				self.options.stack_text_y_offset or 0
			)
	...
	self,
		...
		if (not identifierKey or identifierKey == "") then return end
		...
		if spellIcon then
			...
			if startTime then
				...
				if self.options.show_text then
					...
					local formattedTime =
						(iconFrame.timeRemaining > 0) and (self.options.decimal_timer and iconFrame.parentIconRow.FormatCooldownTimeDecimal(
							iconFrame.timeRemaining
						) or iconFrame.parentIconRow.FormatCooldownTime(iconFrame.timeRemaining)) or ""
			...
			if (descText and self.options.desc_text) then
				...
				iconFrame.Desc:SetTextColor(
					detailsFramework:ParseColors(descText.text_color or self.options.desc_text_color)
				)
				iconFrame.Desc:SetPoint(
					self.options.desc_text_anchor or "bottom",
					iconFrame,
			...
			self:SetWidth(
				(self.options.left_padding * 2) + (self.options.icon_padding * (self.NextIcon - 2)) + (self.options.icon_width * (self.NextIcon - 1))
			)
	...
	FormatCooldownTimeDecimal = function(formattedTime)
		...
		elseif formattedTime < 86400 then
			...
			return ("%dd %02dh"):format(
				formattedTime / 86400,
				(formattedTime / 3600) - (math.floor(formattedTime / 86400) * 24)
			)
		end
	end,
	---@param self df_iconrow the parent frame
	...
	RemoveSpecificIcon = function(self, identifierKey)
		...
		if not self.AuraCache[identifierKey] then return end
	...
	AlignAuraIcons = function(self)
		...
		if iconAmount == 0 then
			...
			table.sort(iconPool, function(i1, i2)
...
function detailsFramework:CreateIconRow(parent, name, options)
	...
	newIconRowFrame:SetSize(
		newIconRowFrame.options.icon_width,
		newIconRowFrame.options.icon_height + (newIconRowFrame.options.top_padding * 2)
	)
