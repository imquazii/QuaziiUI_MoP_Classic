...
local checkPointCallback = function(iconFrame)
	if (iconFrame.timeRemaining < 3) then  end
	return true
end
...
detailsFramework.IconGenericMixin = {
	...
	CreateIcon = function(self, iconName)
		...
		local cooldownFrame =
			CreateFrame("cooldown", "$parentCooldownFrame", newIcon, "CooldownFrameTemplate, BackdropTemplate")
	...
	AddSpecificIconWithTemplate = function(self, iconTemplateTable)
		self:AddSpecificIcon(
			iconTemplateTable.id,
			iconTemplateTable.id,
			nil,
			iconTemplateTable.startTime,
			iconTemplateTable.duration,
			nil,
			nil,
			iconTemplateTable.count,
			nil,
			nil,
			nil,
			nil,
	...
	self,
		...
		if spellIcon then
			...
			do
				...
				PixelUtil.SetPoint(
					iconFrame.CooldownBrightnessTexture,
					"bottomright",
					iconFrame.Texture,
					"bottomright",
					0,
	...
	SetCooldown = function(self, iconFrame)
		...
		if iconFrame.options.remove_on_finish then
			...
			local newLooper =
				detailsFramework.Schedules.NewLooper(
					0.25,
					self.OnIconTick,
					amountOfLoops,
					loopEndCallback,
	...
	OnIconTick = function(iconFrame)
		...
		local percent =
			(((now - iconFrame.startTime) / (iconFrame.modRate or 1)) / (iconFrame.duration / (iconFrame.modRate or 1))) or 0
		local options = iconFrame.options
		...
		if options.show_horizontal_swipe then
			...
			PixelUtil.SetPoint(
				...
				0,
				newHeight
			) --iconFrame.textureHeight -
			local left, right, top, bottom = unpack(iconFrame.CooldownBrightnessTexture.cords)
			local newBottomCord =
				Lerp(
					iconFrame.CooldownBrightnessTexture.top,
					iconFrame.CooldownBrightnessTexture.bottom,
					abs(percent - 1)
				)
		...
		if options.show_text then
			iconFrame.CountdownText:SetText(
				iconFrame.parentIconRow.FormatCooldownTime(
					(iconFrame.duration - (now - iconFrame.startTime)) / (iconFrame.modRate or 1)
				)
			)
				...
				if (growDirection == 1) then --grow to right
					if bIsFirstIcon then
						if self.options.first_icon_use_anchor then
							...
							local attachSide =
								(bIsCenterAligned and "center") or (nWhichSide and not detailsFramework.SideIsCorner[nWhichSide] and "left") or "bottomleft"
				...
				elseif (growDirection == 2) then --grow to left
					if bIsFirstIcon then
						if self.options.first_icon_use_anchor then
							...
							local attachSide =
								(bIsCenterAligned and "center") or (nWhichSide and not detailsFramework.SideIsCorner[nWhichSide] and "right") or "bottomright"
							PixelUtil.SetPoint(iconFrame, attachSide, anchorTo, attachSide, 0, 0)
						end
					else
