...
ButtonMetaFunctions.__call = function(self)
	...
	detailsFramework:CoreDispatch(
		(frameWidget:GetName() or "Button") .. ":__call()",
		self.func,
		frameWidget,
		"LeftButton",
		self.param1,
...
local gmember_texture = function(object)
	return {
		object.button:GetNormalTexture(),
		object.button:GetHighlightTexture(),
		object.button:GetPushedTexture(),
		object.button:GetDisabledTexture(),
	}
end
...
function ButtonMetaFunctions:SetIcon(
texture,
	width,
	height,
	layout,
	...
	local left, right, top, bottom =
		texcoord and texcoord[1],
		texcoord and texcoord[2],
		texcoord and texcoord[3],
		texcoord and texcoord[4]
	texture, width, height, left, right, top, bottom, red, green, blue, alpha =
		detailsFramework:ParseTexture(texture, width, height, left, right, top, bottom, red, green, blue, alpha)
...
function ButtonMetaFunctions:Exec()
	local frameWidget = self.widget
	detailsFramework:CoreDispatch(
		(frameWidget:GetName() or "Button") .. ":Exec()",
		self.func,
		frameWidget,
		"LeftButton",
		self.param1,
...
function ButtonMetaFunctions:Click()
	local frameWidget = self.widget
	detailsFramework:CoreDispatch(
		(frameWidget:GetName() or "Button") .. ":Click()",
		self.func,
		frameWidget,
		"LeftButton",
		self.param1,
...
function ButtonMetaFunctions:RightClick()
	local frameWidget = self.widget
	detailsFramework:CoreDispatch(
		(frameWidget:GetName() or "Button") .. ":RightClick()",
		self.funcright,
		frameWidget,
		"RightButton",
		self.param1,
	...
	if object.options.OnGrab then
		if (type(object.options.OnGrab) == "string" and object.options.OnGrab == "PassClick") then
			if (buttontype == "LeftButton") then
				detailsFramework:CoreDispatch(
					(button:GetName() or "Button") .. ":OnMouseDown()",
					object.func,
					button,
					buttontype,
					object.param1,
...
local OnMouseUp = function(button, buttonType)
	...
	if ((x == button.x and y == button.y) or (button.mouse_down + 0.5 > GetTime() and button:IsMouseOver())) then
		if (buttonType == "LeftButton") then
			detailsFramework:CoreDispatch(
				(button:GetName() or "Button") .. ":OnMouseUp()",
				object.func,
				button,
				buttonType,
				object.param1,
...
function ButtonMetaFunctions:SetTemplate(template)
	...
	if template.rounded_corner then
		...
		if self.__iscolorpicker then
			...
			self.widget.texture_disabled:SetTexture(
				[[Interface\CHARACTERFRAME\TempPortraitAlphaMaskSmall]],
				"CLAMP",
				"CLAMP",
				"TRILINEAR"
...
---@return df_button
function detailsFramework:CreateButton(
parent,
	callback,
	width,
	height,
	...
	return detailsFramework:NewButton(
		parent,
		parent,
		name,
		member,
...
---@return df_button
function detailsFramework:NewButton(
parent,
	container,
	name,
	member,
...
local pickcolorCallback = function(self, red, green, blue, alpha, button)
	...
	--safecall
	detailsFramework:CoreDispatch(
		(self:GetName() or "ColorPicker") .. ".pickcolor_callback()",
		button.MyObject.color_callback,
		button.MyObject,
		red,
		green,
		blue,
