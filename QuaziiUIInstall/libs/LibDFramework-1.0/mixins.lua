...
detailsFramework.SetPointMixin = {SetPoint = function(object, anchorName1, anchorObject, anchorName2, xOffset, yOffset)
	...
	anchorName1, anchorObject, anchorName2, xOffset, yOffset =
		detailsFramework:CheckPoints(anchorName1, anchorObject, anchorName2, xOffset, yOffset, object)
...
detailsFramework.ScriptHookMixin = {
	...
	SetHook = function(self, hookType, func)
		if self.HookList[hookType] then
			...
			if detailsFramework.debug then
				error(
					"Details! Framework: unknown hook type for widget " .. self.WidgetType .. ": '" .. hookType .. "'."
				)
			end
		end
	end,
...
---@class df_sortmixin
detailsFramework.SortFunctions = {---sort a table by a member
---@param self table
...
detailsFramework.StatusBarFunctions = {
	...
	SetMaskTexture = function(self, ...)
		if not self:HasTextureMask() then return end
	...
	CreateTextureMask = function(self)
		...
		if not barTexture then
			detailsFramework:Msg(
				"Object doesn't not have a statubar texture, create one and object:SetStatusBarTexture(textureObject)",
				debugstack()
			)
			return
		end
	...
	HasTextureMask = function(self)
		if not self.barTextureMask then
			detailsFramework:Msg(
				"Object doesn't not have a texture mask, create one using object:CreateTextureMask()",
				debugstack()
			)
			return false
		end
...
detailsFramework.FrameFunctions = {CreateTexture = function(self, name, drawLayer, templateName, subLevel)
