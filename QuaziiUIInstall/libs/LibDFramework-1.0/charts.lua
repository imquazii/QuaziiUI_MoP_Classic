...
---@return boolean
local createAxesLines = function(
self,
	xOffset,
	yOffset,
	whichSide,
...
local updateLabelValues = function(self)
	...
	for i = 1, verticalLabelCount do
		...
		label.circleTexture:SetPoint(
			"center",
			self.yAxisLine,
			"bottomleft",
			-2,
			heightStep * i - 5 + self.chartBottomOffset
		)
...
detailsFramework.ChartFrameSharedMixin = {
	...
	CreateBackdropIndicator = function(self, nextIndicatorIndex)
		...
		local newBackdropIndicator =
			CreateFrame("frame", "$parentBackdropIndicator" .. nextIndicatorIndex, self.plotFrame)
	...
	ShowBackdropIndicators = function(self)
		...
		for i = 1, self.nextBackdropIndicator - 1 do
			...
			if (i == 1) then
				...
				thisIndicator.indicatorTexture:SetPoint(
					"left",
					self.backdropIndicators[i - 1].indicatorTexture,
					"left",
					-space,
					0
				)
