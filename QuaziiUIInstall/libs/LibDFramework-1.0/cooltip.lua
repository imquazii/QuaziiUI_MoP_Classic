...
if (not DF or not DetailsFrameworkCanLoad) then return end
...
function DF:CreateCoolTip()
	...
	if (_G.GameCooltip2 and _G.GameCooltip2.version >= version) then return end
	...
	function gameCooltip:ParseMenuType(menuType)
		if ((type(menuType) == "number" and menuType == 1) or (type(
			menuType
		) == "string" and menuType == CONST_MENU_TYPE_MAINMENU)) then
			return CONST_MENU_TYPE_MAINMENU
		end

		if ((type(menuType) == "number" and menuType == 2) or (type(
			menuType
		) == "string" and menuType == CONST_MENU_TYPE_SUBMENU)) then
	...
	local createTooltipFrames = function(self)
		...
		if not self.gradientTexture then
			self.gradientTexture = DetailsFramework:CreateTexture(
				self,
				{
					gradient = "vertical",
					fromColor = {0, 0, 0, .2},
	...
	function GameCooltip:ShowRoundedCorner()
		if not frame1.HaveRoundedCorners then return end
	...
	local OnClickFunctionMainButton = function(self, button)
		...
		if gameCooltip.FunctionsTableMain[self.index] then
			...
			local okay, errortext =
				xpcall(
					func,
					geterrorhandler(),
					gameCooltip.Host,
					gameCooltip.FixedValue,
					parameterTable[1],
					parameterTable[2],
					parameterTable[3],
					button
	...
	function gameCooltip:TextAndIcon(
	index,
		frame,
		menuButton,
		leftTextSettings,
		...
		menuButton.rightText:SetPoint(
			"right",
			menuButton.rightIcon,
			"left",
			gameCooltip.OptionsTable.RightTextMargin or -3,
			0
		)
		...
		if leftTextSettings then
			...
			elseif leftTextSettings[7] then
				...
				menuButton.leftText:SetFont(
					gameCooltip.defaultFont,
					leftTextSettings[6] or gameCooltip.OptionsTable.TextSize or 10,
					leftTextSettings[8] or gameCooltip.OptionsTable.TextShadow
				)
			end
			...
			elseif gameCooltip.OptionsTable.TextActuallyShadow then
				local shadow_r, shadow_g, shadow_b, shadow_a =
					DF:ParseColors(gameCooltip.OptionsTable.TextActuallyShadow)
				menuButton.leftText:SetShadowColor(shadow_r, shadow_g, shadow_b, shadow_a)
			else
		...
		if rightTextSettings then
			...
			elseif rightTextSettings[7] then
				...
				menuButton.rightText:SetFont(
					gameCooltip.defaultFont,
					rightTextSettings[6] or gameCooltip.OptionsTable.TextSize or 10,
					rightTextSettings[8] or gameCooltip.OptionsTable.TextShadow
				)
			end
		...
		if (leftIconSettings and leftIconSettings[1]) then
			...
			if (type(
				leftIconSettings[1]
			) == "table" and leftIconSettings[1].GetObjectType and leftIconSettings[1]:GetObjectType() == "Texture") then
		...
		if (rightIconSettings and rightIconSettings[1]) then
			...
			if (type(
				rightIconSettings[1]
			) == "table" and rightIconSettings[1].GetObjectType and rightIconSettings[1]:GetObjectType() == "Texture") then
			...
			menuButton.rightIcon:SetTexCoord(
				rightIconSettings[4],
				rightIconSettings[5],
				rightIconSettings[6],
				rightIconSettings[7]
		...
		if not isSecondFrame then --main frame
			if not gameCooltip.OptionsTable.FixedWidth then
				if (gameCooltip.Type == 1 or gameCooltip.Type == 2) then
					local stringWidth =
						menuButton.leftText:GetStringWidth() + menuButton.rightText:GetStringWidth() + menuButton.leftIcon:GetWidth() + menuButton.rightIcon:GetWidth() + 10
					if (stringWidth > frame.w) then
						frame.w = stringWidth
					end
				end
			else
				menuButton.leftText:SetWidth(
					gameCooltip.OptionsTable.FixedWidth - menuButton.leftIcon:GetWidth() - menuButton.rightText:GetStringWidth() - menuButton.rightIcon:GetWidth() - 22
				)
			end
		else
			if not gameCooltip.OptionsTable.FixedWidthSub then
				if (gameCooltip.Type == 1 or gameCooltip.Type == 2) then
					local stringWidth =
						menuButton.leftText:GetStringWidth() + menuButton.rightText:GetStringWidth() + menuButton.leftIcon:GetWidth() + menuButton.rightIcon:GetWidth()
					if (stringWidth > frame.w) then
						frame.w = stringWidth
					end
				end
				...
				menuButton.leftText:SetWidth(
					gameCooltip.OptionsTable.FixedWidthSub - menuButton.leftIcon:GetWidth() - 12
				)
			end
		end
		...
		local height =
			max(
				menuButton.leftIcon:GetHeight(),
				menuButton.rightIcon:GetHeight(),
				menuButton.leftText:GetStringHeight(),
				menuButton.rightText:GetStringHeight()
			)
	...
	function gameCooltip:RefreshSpark(menuButton)
		...
		menuButton.spark:SetPoint(
			"left",
			menuButton.statusbar,
			"left",
			(menuButton.statusbar:GetValue() * (menuButton.statusbar:GetWidth() / 100)) - 5 + positionXOffset,
			0 + positionYOffset
		)
		menuButton.spark2:ClearAllPoints()
		menuButton.spark2:SetPoint(
			"left",
			menuButton.statusbar,
			"left",
			menuButton.statusbar:GetValue() * (menuButton.statusbar:GetWidth() / 100) - 16 + positionXOffset,
			0 + positionYOffset
		)
	end
	...
	function gameCooltip:StatusBar(menuButton, statusBarSettings)
		if statusBarSettings then
			...
			menuButton.statusbar:SetStatusBarColor(
				statusBarSettings[2],
				statusBarSettings[3],
				statusBarSettings[4],
				statusBarSettings[5]
			...
			if statusBarSettings[7] then
				...
				menuButton.statusbar2.texture:SetTexture(
					statusBarSettings[7].texture or [[Interface\RaidFrame\Raid-Bar-Hp-Fill]]
				)
		...
		if gameCooltip.OptionsTable.RightBorderSize then
			menuButton.statusbar:SetPoint(
				"right",
				menuButton,
				"right",
				gameCooltip.OptionsTable.RightBorderSize + -10,
				0
			)
		else
	...
	function gameCooltip:SetupMainButton(menuButton, index)
		...
		gameCooltip:TextAndIcon(
			index,
			frame1,
			menuButton,
			gameCooltip.LeftTextTable[index],
			gameCooltip.RightTextTable[index],
			gameCooltip.LeftIconTable[index],
			gameCooltip.RightIconTable[index]
		)
		--setup statusbar
	...
	function gameCooltip:SetupButtonOnSecondFrame(menuButton, index, mainMenuIndex)
		...
		gameCooltip:TextAndIcon(
			...
			frame2,
			menuButton,
			gameCooltip.LeftTextTableSub[mainMenuIndex] and gameCooltip.LeftTextTableSub[mainMenuIndex][index],
			...
			gameCooltip.RightIconTableSub[mainMenuIndex] and gameCooltip.RightIconTableSub[mainMenuIndex][index],
			true
		)

		--setup statusbar
		gameCooltip:StatusBar(
			menuButton,
			gameCooltip.StatusBarTableSub[mainMenuIndex] and gameCooltip.StatusBarTableSub[mainMenuIndex][index]
		)
		...
		for i = 1, gameCooltip.IndexesSub[index] do
			...
			if (menuButton.leftText:GetText() == "$div") then
				...
				menuButton:SetPoint(
					...
					"top",
					0,
					(((i - 1) * frame2.hHeight) * -1) - 4 + (gameCooltip.OptionsTable.ButtonsYModSub or 0) + spacing + (2 + (divisorOffsetTop or 0))
				)
				...
				menuButton:SetPoint(
					...
					"top",
					0,
					(((i - 1) * frame2.hHeight) * -1) - 4 + (gameCooltip.OptionsTable.ButtonsYModSub or 0) + spacing
				)
	...
	function gameCooltip:LeftTextSpace(row)
		row.leftText:SetWidth(
			row:GetWidth() - 30 - row.leftIcon:GetWidth() - row.rightIcon:GetWidth() - row.rightText:GetStringWidth()
		)
	...
	function gameCooltip:BuildTooltipSecondFrame()
		...
		for currentIndex = 1, #LeftTextTableSub do
			...
			gameCooltip:TextAndIcon(
				...
				frame2,
				button,
				LeftTextTableSub[currentIndex],
				RightTextTableSub[currentIndex],
				LeftIconTableSub[currentIndex],
				RightIconTableSub[currentIndex]
			)
			--setup statusbar
		...
		--normalize height of all rows
		local heightValue =
			(gameCooltip.OptionsTable.TopBorderSize or -6) + spacing + (gameCooltip.OptionsTable.ButtonsYMod or 0)
		...
		for i = 1, #LeftTextTableSub do
			...
			if gameCooltip.OptionsTable.AlignAsBlizzTooltip then
				local height =
					max(
						...
						menuButton.leftIcon:GetHeight(),
						menuButton.rightIcon:GetHeight(),
						gameCooltip.OptionsTable.AlignAsBlizzTooltipForceHeight or 2
					)
			...
			elseif gameCooltip.OptionsTable.IgnoreButtonAutoHeight then
				...
				menuButton:SetPoint(
					...
					"top",
					0,
					(((i - 1) * frame2.hHeight) * -1) + (gameCooltip.OptionsTable.TopBorderSize or -6) + (gameCooltip.OptionsTable.ButtonsYMod or 0) + spacing
				)
			end
		...
		if gameCooltip.OptionsTable.FixedHeight then
			...
			if gameCooltip.OptionsTable.AlignAsBlizzTooltip then
				local newHeight =
					((heightValue - 10) * -1) + (gameCooltip.OptionsTable.AlignAsBlizzTooltipFrameHeightOffset or 0)
			...
			elseif gameCooltip.OptionsTable.IgnoreButtonAutoHeight then
				...
				local newHeight =
					(frame2.hHeight * gameCooltip.Indexes) + 8 + ((gameCooltip.OptionsTable.ButtonsYMod or 0) * -1)
	...
	function gameCooltip:SetSpellByID(
	spellId,
		bShowDescriptionOnly --~spell
	)
		if (type(spellId) == "number") then
			...
			if spellName then
				...
				local chargesAvailable, maxCharges, chargeCooldownStart, rechargeTime, chargeModRate =
				...
				if not bShowDescriptionOnly then
					if (tResourceCost.cost and tResourceCost.cost > 0) then
						if (maxRange and maxRange > 0) then
							gameCooltip:AddLine(
								tResourceCost.cost .. " " .. (_G[tResourceCost.name] or tResourceCost.name),
								string.format(_G.SPELL_RANGE, math.floor(maxRange)),
								1,
								1,
								1,
								1,
					...
					if (castTime and castTime > 0) then
						...
						if (cooldownTime and cooldownTime > 0) then
							gameCooltip:AddLine(
								string.format(_G.SPELL_CAST_TIME_SEC, castTime),
								string.format(_G.SPELL_RECAST_TIME_SEC, cooldownTime),
								1,
								1,
								1,
								1,
						...
						elseif (rechargeTime and rechargeTime > 0) then
							gameCooltip:AddLine(
								string.format(_G.SPELL_CAST_TIME_SEC, castTime),
								string.format(_G.SPELL_RECAST_TIME_CHARGES_SEC, rechargeTime),
								1,
								1,
								1,
								1,
					...
					elseif (castTime == 0) then --spell is instant (has no cast time)
						if (cooldownTime and cooldownTime > 0) then
							gameCooltip:AddLine(
								_G.SPELL_CAST_TIME_INSTANT,
								string.format(_G.SPELL_RECAST_TIME_SEC, cooldownTime / 1000),
								1,
								1,
								1,
								1,
	...
	function gameCooltip:BuildTooltip() --~refresh
		...
		for i = 1, gameCooltip.Indexes do
			...
			gameCooltip:TextAndIcon(
				...
				frame1,
				button,
				gameCooltip.LeftTextTable[i],
				gameCooltip.RightTextTable[i],
				gameCooltip.LeftIconTable[i],
				gameCooltip.RightIconTable[i]
			)
			--setup statusbar
		...
		for i = 1, gameCooltip.Indexes do
			...
			elseif gameCooltip.OptionsTable.IgnoreButtonAutoHeight then
				...
				menuButton:SetPoint(
					...
					"top",
					0,
					(((i - 1) * frame1.hHeight) * -1) + (gameCooltip.OptionsTable.TopBorderSize or -6) + (gameCooltip.OptionsTable.ButtonsYMod or 0) + spacing
				)
			end
		...
		if gameCooltip.OptionsTable.FixedHeight then
			...
			elseif gameCooltip.OptionsTable.IgnoreButtonAutoHeight then
				...
				local newHeight =
					(frame1.hHeight * gameCooltip.Indexes) + 8 + ((gameCooltip.OptionsTable.ButtonsYMod or 0) * -1)
		...
		for i = 1, gameCooltip.Indexes do
			...
			if (menuButton.leftText:GetText() == "$div") then
				...
				menuButton:SetPoint(
					...
					"top",
					0,
					(((i - 1) * frame1.hHeight) * -1) - 4 + (gameCooltip.OptionsTable.ButtonsYMod or 0) + spacing - 4 + divisorOffsetTop
				)
				if gameCooltip.OptionsTable.YSpacingMod then
					spacing = spacing + gameCooltip.OptionsTable.YSpacingMod
				end
				...
				menuButton:SetPoint(
					...
					"top",
					0,
					(((i - 1) * frame1.hHeight) * -1) - 4 + (gameCooltip.OptionsTable.ButtonsYMod or 0) + spacing
				)
				if gameCooltip.OptionsTable.YSpacingMod then
					spacing = spacing + gameCooltip.OptionsTable.YSpacingMod
				end
	...
	function gameCooltip:SetMyPoint(host, xOffset, yOffset)
		...
		PixelUtil.SetPoint(
			frame1,
			gameCooltip.OptionsTable.MyAnchor,
			anchor,
			gameCooltip.OptionsTable.RelativeAnchor,
			0 + moveX + gameCooltip.OptionsTable.WidthAnchorMod,
			10 + gameCooltip.OptionsTable.HeightAnchorMod + moveY
		)
		...
		if (bHadXPositionOutOfScreen or bHadYPositionOutOfScreen) then
			return gameCooltip:SetMyPoint(
				host,
				bHadXPositionOutOfScreen and xOffset or 0,
				bHadYPositionOutOfScreen and yOffset or 0
			)
		end
	...
	function gameCooltip:AddStatusBar(
	statusbarValue,
		menuType,
		colorRed,
		colorGreen,
		...
		--check data integrity
		if (type(statusbarValue) ~= "number") then return end

		menuType = gameCooltip:ParseMenuType(menuType)

		if (type(colorRed) == "table" or type(colorRed) == "string") then
			statusbarGlow, backgroundBar, colorRed, colorGreen, colorBlue, colorAlpha =
				colorGreen,
				colorBlue,
				DF:ParseColors(colorRed)
	...
	function gameCooltip:SetWallpaper(menuType, texture, texcoord, color, bDesaturated, desaturation)
		...
		local iconTexture,
			...
			blue,
			alpha
		= detailsFramework:ParseTexture(texture, 1, 1, texcoord[1], texcoord[2], texcoord[3], texcoord[4], color)
	...
	function gameCooltip:AddTexture(
	iconTexture,
		menuType,
		side,
		iconWidth,
	...
	iconTexture,
		...
		iconTexture, iconWidth, iconHeight, leftCoord, rightCoord, topCoord, bottomCoord, red, green, blue, alpha =
			detailsFramework:ParseTexture(
				iconTexture,
				iconWidth,
				iconHeight,
				leftCoord,
		...
		elseif (menuType == CONST_MENU_TYPE_SUBMENU) then
			...
			elseif ((type(side) == "string" and side == "top") or (type(side) == "number" and side == 3)) then
				gameCooltip.TopIconTableSub[gameCooltip.Indexes] =
					gameCooltip.TopIconTableSub[gameCooltip.Indexes] or {}
	...
	function gameCooltip:AddPopUpFrame(onShowFunc, onHideFunc, param1, param2)
		--act like a sub menu
		if (gameCooltip.Indexes > 0) then
			gameCooltip.PopupFrameTable[gameCooltip.Indexes] =
				{onShowFunc or false, onHideFunc or false, param1, param2}
		end
	end
	...
	function gameCooltip:AddDoubleLine(
	leftText,
		rightText,
		menuType,
		ColorR1,
		...
		textWidth,
		textHeight,
		textContour --ãddline ~addline
	)
		return gameCooltip:AddLine(
			leftText,
			rightText,
			menuType,
			ColorR1,
	...
	leftText,
		...
		if (type(ColorR1) ~= "number") then
			ColorR2, ColorG2, ColorB2, ColorA2, fontSize, fontFace, fontFlag, textWidth, textHeight, textContour =
				ColorG1,
				ColorB1,
				ColorA1,
				ColorR2,
		...
		if (type(ColorR2) ~= "number") then
			fontSize, fontFace, fontFlag, textWidth, textHeight, textContour =
				ColorG2,
				ColorB2,
				ColorA2,
				fontSize,
	...
	--old function call
	function gameCooltip:ShowMe(
	host,
		arg2 --drunk code
	)
		--ignore if mouse is within the frame region
		if gameCooltip.mouseOver then return end
		...
		if openOnClick then
			if (host:GetObjectType() == "Button") then
				host:SetScript("OnClick", function()
					gameCooltip:ExecFunc(host, true)
				end)
			end
		end
