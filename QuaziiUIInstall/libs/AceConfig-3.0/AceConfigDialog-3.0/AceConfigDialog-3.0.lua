...
AceConfigDialog.tooltip =
	AceConfigDialog.tooltip or CreateFrame("GameTooltip", "AceConfigDialogTooltip", UIParent, "GameTooltipTemplate")
...
local function GetOptionsMemberValue(membername, option, options, path, appName, ...)
	...
	if not allIsLiteral[membername] and (type(member) == "function" or (not stringIsLiteral[membername] and type(
		member
	) == "string")) then
...
function AceConfigDialog:FeedGroup(appName, options, container, rootframe, path, isRoot)
	...
	for k, v in pairs(group.args) do
		if v.type == "group" and not pickfirstset(
			v.dialogInline,
			v.guiInline,
			v.inline,
			false
		) and not CheckOptionHidden(v, options, path, appName) then
			hasChildGroups = true
		end
	end
	if group.plugins then
				...
				if not entry.disabled then
					tab:SelectTab(
						(GroupExists(
							...
							options,
							path,
							status.groups.selected
						) and status.groups.selected) or entry.value
					)
					break
				end
			end
			...
			if firstgroup then
				selectGroup:SetGroup(
					(GroupExists(
						...
						path,
						status.groups.selected
					) and status.groups.selected) or firstgroup
				)
			end
