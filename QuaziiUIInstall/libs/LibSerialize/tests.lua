...
do
	local serialized = LibSerialize:SerializeEx({errorOnUnserializableType = false}, print, {
		a = 1,
		b = print,
	})
...
do
	...
	t.nested = nested
	setmetatable(nested, {
		__LibSerialize = {filter = function(t, k, v)
			return k ~= "c"
		end},
	})
	local opts = {filter = function(t, k, v)
		return LibSerialize:IsSerializableType(k, v)
	end}
...
do
	...
	local opts = {
		stable = true,
		filter = function(t, k, v)
			return not tonumber(k) or tonumber(k) < 100
		end,
	}
...
local failCases = {{print}, {[print] = true}, {[true] = print}, print}
